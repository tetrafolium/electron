...
let postData
  ...
  postData = [
    {type: 'rawData', bytes: Buffer.from('username=test&file=')}, {
      type: 'file',
      filePath: filePath,
      offset: 0,
  ...
  after(() => {
    ...
    afterEach(() => {return closeWindow(w).then(() => {w = null})})
  ...
  switch (request.url) {
    case '/404':
      ...
      response.end('hello')
      break case '/title': response.statusCode = '200'
  ...
  describe('BrowserWindow.setContentBounds(bounds)', () => {
    ...
    it('works for a frameless window', (done) => {
      w.destroy()
    w = new BrowserWindow({show: false, frame: false, width: 300, height: 300})
      const bounds = {x: 10, y: 10, width: 250, height: 250}
      w.once('resize', () => {
  ...
  it('sets the progress using "paused" mode',
     () => {
         assert.doesNotThrow(() => {w.setProgressBar(0.5, {mode: 'paused'})})})
  it('sets the progress using "error" mode',
     () => {
         assert.doesNotThrow(() => {w.setProgressBar(0.5, {mode: 'error'})})})
  ...
  assert.throws(() => {w.setAlwaysOnTop(true, '', -2147483644)})

  assert.throws(() => {w.setAlwaysOnTop(true, '', 2147483632)})
    })
  })
  ...
  if (process.platform !== 'darwin') {
    ...
    assert.doesNotThrow(() => {w.addTabbedWindow(tabbedWindow)})
  ...
  afterEach(() => {contents.destroy()})
    ...
    assert.equal(
        BrowserWindow.fromDevToolsWebContents(w.webContents), undefined)
  ...
  describe('BrowserWindow.setOpacity(opacity)', () => {
    it('make window with initial opacity', () => {
      w.destroy()
  w = new BrowserWindow({show: false, width: 400, height: 400, opacity: 0.5})
  ...
  describe('"useContentSize" option', () => {
    it('make window created with content size when used', () => {
      w.destroy()
  w = new BrowserWindow(
      {show: false, width: 400, height: 400, useContentSize: true})
  ...
  if (parseInt(os.release().split('.')[0]) < 14) {
    ...
    it('creates browser window with hidden title bar', () => {
      w.destroy()
    w = new BrowserWindow(
        {show: false, width: 400, height: 400, titleBarStyle: 'hidden'})
    const contentSize = w.getContentSize()
      assert.equal(contentSize[1], 400)
    })
    it('creates browser window with hidden inset title bar', () => {
      w.destroy()
    w = new BrowserWindow(
        {show: false, width: 400, height: 400, titleBarStyle: 'hidden-inset'})
    const contentSize = w.getContentSize()
      assert.equal(contentSize[1], 400)
    })
  })
  ...
  if (process.platform !== 'darwin') {
    ...
    it('sets the window width to the page width when used', () => {
      w.destroy()
    w = new BrowserWindow(
        {show: false, width: 500, height: 400, zoomToPageWidth: true})
    w.maximize()
  ...
  describe('"tabbingIdentifier" option', () => {
    it('can be set on a window', () => {
      w.destroy()
  w = new BrowserWindow({tabbingIdentifier: 'group1'})
  w.destroy()
      w = new BrowserWindow({
    ...
    describe('"preload" option', () => {
      it('loads the script before other scripts in window', (done) => {
        ...
        ipcMain.once('answer', (event, test) => {
          ...
          w.destroy()
          w = new BrowserWindow(
              {show: false, webPreferences: {preload: preload}})
      ...
      beforeEach(() => {
        ...
        afterEach(() => {defaultSession.setPreloads([])})

        it('can set multiple session preload script', function() {
          assert.deepEqual(defaultSession.getPreloads(), preloads)
        })

        it('loads the script before other scripts in window including normal preloads',
           function(done) {
             ipcMain.once(
                 'vars', function(event, preload1, preload2, preload3) {
             ...
             w = new BrowserWindow({
               ...
               webPreferences: {
                 preload:
                     path.join(fixtures, 'module', 'set-global-preload-3.js')
               }
             })
    ...
    describe('"node-integration" option', () => {
      it('disables node integration when specified to false', (done) => {
        ...
        assert.equal(typeofBuffer, 'undefined')
          ...
          w = new BrowserWindow({
            show: false,
            webPreferences: {preload: preload, nodeIntegration: false}
          })
  ...
  before(
      (done) => {protocol.interceptStringProtocol(
          'http', crossDomainHandler, () => {done()})})

  after((done) => {protocol.uninterceptProtocol('http', () => {done()})})
  ...
  assert.equal(test, 'preload')
      ...
      w.destroy()
      w = new BrowserWindow(
          {show: false, webPreferences: {sandbox: true, preload: preload}})
        w.loadURL('file://' + path.join(fixtures, 'api', 'preload.html'))
      })
  ...
  assert.equal(url, expectedUrl)
      ...
      w = new BrowserWindow(
          {show: false, webPreferences: {sandbox: true, preload: preload}})
      ipcRenderer.send(
          'set-web-preferences-on-next-new-window', w.webContents.id, 'preload',
          preload)
  ...
  ipcMain.once('answer', function(event, html) {
    assert.equal(html, '<h1>scripting from opener</h1>')
      ...
      ipcRenderer.send(
          ...
          preload)
      let htmlPath =
          path.join(fixtures, 'api', 'sandbox.html?window-open-external')
      const pageUrl = 'file://' + htmlPath
      let popupWindow
      w.loadURL(pageUrl)
        w.webContents.once('new-window', (e, url, frameName, disposition, options) => {
  ...
  popupWindow = window
      ...
      ipcRenderer.send(
          ...
          preloadPath)
      ipcRenderer.send(
          'set-web-preferences-on-next-new-window', w.webContents.id, 'foo',
          'bar')
      ...
      w = new BrowserWindow(
          {show: false, webPreferences: {sandbox: true, preload: preload}})
      let htmlPath =
          path.join(fixtures, 'api', 'sandbox.html?verify-ipc-sender')
      const pageUrl = 'file://' + htmlPath
      w.loadURL(pageUrl)
        w.webContents.once('new-window', (e, url, frameName, disposition, options) => {
  ...
  assert.notEqual(data.length, 0)
      ...
      const initialWebContents =
          webContents.getAllWebContents().map((i) => i.id)
      ...
      it('releases memory after popup is closed', (done) => {
        w.destroy()
      w = new BrowserWindow(
          {show: false, webPreferences: {preload: preload, sandbox: true}})
      w.loadURL(
          'file://' +
          path.join(fixtures, 'api', 'sandbox.html?allocate-memory'))
  ...
  assert(memoryDecreaseByClose > memoryIncreaseByOpen * 0.7)
      ...
      w = new BrowserWindow(
          {show: false, webPreferences: {preload: preload, sandbox: true}})
      w.loadURL(
          'file://' + path.join(fixtures, 'api', 'sandbox.html?reload-remote'))

      ipcMain.on(
          'get-remote-module-path',
          (event) => {
      ...
      let reload = false
        ipcMain.on('reloaded', (event) => {
          ...
          ipcMain.once('reload', (event) => {event.sender.reload()})
      ...
      w.loadURL(
          'file://' +
          path.join(fixtures, 'api', 'sandbox.html?reload-remote-child'))

      ipcMain.on(
          'get-remote-module-path',
          (event) => {
              event.returnValue =
                  path.join(fixtures, 'module', 'hello-child.js')})
  ...
  if (!nativeModulesEnabled) {
    return done()
        ...
        ipcMain.once('answer', (event, content) => {
          ...
          w.loadURL(`file://${
              path.join(
                  fixtures, 'api', 'native-window-open-native-addon.html')}`)
  })
      it('should inherit the nativeWindowOpen setting in opened windows', (done) => {
        w.destroy()
      w = new BrowserWindow(
          {show: false, webPreferences: {nativeWindowOpen: true}})
  ...
  ipcMain.once('pong', (event, visibilityState, hidden) => {
    ...
    w.once('ready-to-show', () => {readyToShow = true})
  ...
  if (!hidden) {
    assert.equal(visibilityState, 'visible')
      ...
      it('visibilityState changes when window is shown inactive',
         function(done) {
           if (isCI && process.platform === 'win32') {
             // FIXME(alexeykuzmin): Skip the test instead of marking it as
             // passed. afterEach hook won't be run if a test is skipped
             // dynamically. If afterEach isn't run current window won't be
             // destroyed and the next test will fail on assertion in
             // `closeWindow()`. this.skip()
             return done()
           }
    ...
    if (!hidden) {
      ...
      w.hide()
        ...
        w.loadURL(
            `file://${path.join(fixtures, 'pages', 'visibilitychange.html')}`)
         })
  ...
  if (isCI && process.platform === 'darwin') {
    ...
    w.webContents.once(
        'new-window',
        function(e, url, frameName, disposition, options, additionalFeatures) {
          e.preventDefault()
          assert.equal(url, 'http://host/')
          assert.equal(frameName, 'host')
          assert.equal(additionalFeatures[0], 'this-is-not-a-standard-feature')
  ...
  afterEach(
      () => {return closeWindow(sheet, {assertSingleWindow: false})
                 .then(() => {sheet = null})})
  ...
  afterEach(
    ...
    it('emits when window has closed a sheet', (done) => {
      w.show()
    sheet = new BrowserWindow({modal: true, parent: w})
  ...
  describe('window states', () => {
    it('does not resize frameless windows when states change', () => {
      w.destroy()
  w = new BrowserWindow({frame: false, width: 300, height: 200, show: false})
    ...
    describe('loading main frame state', () => {
      ...
      it('is true when navigating to pages from the same origin', (done) => {
        ...
        describe('window states (excluding Linux)', () => {
          // FIXME(alexeykuzmin): Skip the tests instead of using the `return`
          // here. Why it cannot be done now:
        ...
        describe(
            ...
            () => {it('should restore the previous window size', () => {
              if (w != null)
                ...
                w = new BrowserWindow({minWidth: 800, width: 800})
          ...
          const show =
              () => {
                if (w == null || w.isDestroyed())
                  return const {devToolsWebContents} = w
                  if (devToolsWebContents == null ||
                      devToolsWebContents.isDestroyed()) {
                    return
                  }
                ...
                const showLastPanel =
                    () => {
                      const lastPanelId =
                          UI.inspectorView._tabbedPane._tabs.peekLast().id
  ...
  afterEach(() => {clearTimeout(showPanelTimeoutId)})

    describe('BrowserWindow.addDevToolsExtension', () => {
      beforeEach(() => {
        BrowserWindow.removeDevToolsExtension('foo')
    assert.equal(
        BrowserWindow.getDevToolsExtensions().hasOwnProperty('foo'), false)

    var extensionPath =
        path.join(__dirname, 'fixtures', 'devtools-extensions', 'foo')
    BrowserWindow.addDevToolsExtension(extensionPath)
    assert.equal(
        BrowserWindow.getDevToolsExtensions().hasOwnProperty('foo'), true)
        ...
        it('throws errors for invalid manifest.json files',
           () => {assert.throws(
                      () => {BrowserWindow.addDevToolsExtension(path.join(
                          __dirname, 'fixtures', 'devtools-extensions',
                          'bad-manifest'))},
                      /Unexpected token }/)})
  ...
  assert.deepEqual(message.storageItems, {
    ...
    sync: {set: {foo: 'bar', bar: 'foo'}, remove: {foo: 'bar'}, clear: {}}
  })
  done()
          })
        })
  ...
  assert.equal(message.tabId, w.webContents.id)
          ...
          it('works when used with partitions', (done) => {
            ...
            w = new BrowserWindow(
                {show: false, webPreferences: {partition: 'temp'}})
    ...
    var extensionPath =
        path.join(__dirname, 'fixtures', 'devtools-extensions', extensionName)
    var serializedPath =
    ...
    app.emit('will-quit')
    assert.deepEqual(
        JSON.parse(fs.readFileSync(serializedPath)), [extensionPath])
  ...
  const code = `(() => "${expected}")()`
  const asyncCode =
      `(() => new Promise(r => setTimeout(() => r("${expected}"), 500)))()`
  const badAsyncCode = `(() => new Promise((r, e) => setTimeout(() => e("${
      expectedErrorMsg}"), 500)))()`
  const errorTypes = new Set([
    Error, ReferenceError, EvalError, RangeError, SyntaxError, TypeError,
    URIError
  ])
  ...
  assert.equal(result, expected)
    ...
    it('rejects the returned promise if an async error is thrown', (done) => {
      ...
      ipcRenderer.once('executeJavaScript-promise-error', (event, error) => {
        ...
        it('rejects the returned promise with an error if an Error.prototype is thrown',
           async () => {
  ...
  const assertWithinDelta =
      (actual, expect, delta, label) => {
        const result = Math.abs(actual - expect)
        assert.ok(
            result <= delta,
            `${label} value of ${actual} was not within ${delta} of ${expect}`)
      }
  ...
  const isScaleFactorRounding = () => {
    ...
    if (Math.round(scaleFactor) !== scaleFactor) {
      return true
    ...
    function serveFileFromProtocol(protocolName, filePath) {
      return new Promise(
          (resolve, reject) => {protocol.registerBufferProtocol(
              protocolName,
              (request, callback) => {
                  // Disabled due to false positive in StandardJS
                  // eslint-disable-next-line standard/no-callback-literal
