...
describe('nativeImage module', () => {
  ...
  const images = [
    ...
    {
      dataUrl:
          'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAMAAAADCAYAAABWKLW/AAAADElEQVQYlWNgIAoAAAAnAAGZWEMnAAAAAElFTkSuQmCC',
      filename: '3x3.png',
      format: ImageFormat.PNG,
      hasAlphaChannel: true,
      hasDataUrl: true,
  ...
  const getImages = (filters) => {
    const matchingImages =
        images.filter((i) => imageMatchesTheFilters(i, filters));
  ...
  describe('createEmpty()', () => {
    it('returns an empty image', () => {
      ...
      expect(empty.toDataURL({
        scaleFactor: 2.0
      })).to.equal('data:image/png;base64,');
  ...
  describe('createFromBuffer(buffer, scaleFactor)', () => {
    ...
    it('returns an image created from the given buffer', () => {
      const imageA = nativeImage.createFromPath(
          path.join(__dirname, 'fixtures', 'assets', 'logo.png'));
    ...
    it('returns an image created from the given string', () => {
      ...
      for (const imageData of imagesData) {
        ...
        const imageFromDataUrl =
            nativeImage.createFromDataURL(imageData.dataUrl);

        expect(imageFromDataUrl.isEmpty());
        expect(imageFromDataUrl.getSize())
            .to.deep.equal(imageFromPath.getSize());
        expect(imageFromDataUrl.toBitmap())
            .to.satisfy((bitmap) => imageFromPath.toBitmap().equals(bitmap));
        expect(imageFromDataUrl.toDataURL())
            .to.equal(imageFromPath.toDataURL());
      }
    });
  });
  ...
  describe('toDataURL()', () => {
    it('returns a PNG data URL', () => {
      ...
      for (const imageData of imagesData) {
        ...
        for (const scaleFactor of scaleFactors) {
          expect(imageFromPath.toDataURL({scaleFactor}))
              .to.equal(imageData.dataUrl);
        }
      }
    });
    ...
    it('supports a scale factor', () => {
      ...
      const imageFromDataUrlOne =
          nativeImage.createFromDataURL(image.toDataURL({scaleFactor: 1.0}));
      expect(imageFromDataUrlOne.getSize()).to.deep.equal(expectedSize);

      const imageFromDataUrlTwo =
          nativeImage.createFromDataURL(image.toDataURL({scaleFactor: 2.0}));
  ...
  describe('toPNG()', () => {
    ...
    it('supports a scale factor', () => {
      ...
      const imageFromBufferOne =
          nativeImage.createFromBuffer(image.toPNG({scaleFactor: 1.0}));
  ...
  describe('createFromPath(path)', () => {
    ...
    it('loads images from paths relative to the current working directory',
       () => {
         const imagePath = `.${path.sep}${
             path.join('spec', 'fixtures', 'assets', 'logo.png')}`;
    ...
    it('loads images from paths with `.` segments', () => {
      const imagePath = `${path.join(__dirname, 'fixtures')}${path.sep}.${
          path.sep}${path.join('assets', 'logo.png')}`;
      const image = nativeImage.createFromPath(imagePath);
      expect(image.isEmpty()).to.be.false;
      expect(image.getSize()).to.deep.equal({width: 538, height: 190});
    });
    ...
    it('loads images from paths with `..` segments', () => {
      const imagePath = `${path.join(__dirname, 'fixtures', 'api')}${
          path.sep}..${path.sep}${path.join('assets', 'logo.png')}`;
      const image = nativeImage.createFromPath(imagePath);
      expect(image.isEmpty()).to.be.false;
      expect(image.getSize()).to.deep.equal({width: 538, height: 190});
    });
    ...
    it('Gets an NSImage pointer on macOS', function() {
      ...
      const allBytesAreNotNull =
          nsimage.reduce((acc, x) => acc || x !== 0, false);
  ...
  describe('crop(bounds)', () => {
    it('returns an empty image when called on an empty image', () => {
      expect(nativeImage.createEmpty()
  ...
  describe('addRepresentation()', () => {
    it('supports adding a buffer representation for a scale factor', () => {
      ...
      expect(image.toDataURL({
        scaleFactor: 1.0
      })).to.equal(imageDataOne.dataUrl);
      expect(image.toDataURL({
        scaleFactor: 2.0
      })).to.equal(imageDataTwo.dataUrl);
      expect(image.toDataURL({
        scaleFactor: 3.0
      })).to.equal(imageDataThree.dataUrl);
      expect(image.toDataURL({
        scaleFactor: 4.0
      })).to.equal(imageDataThree.dataUrl);
    });
