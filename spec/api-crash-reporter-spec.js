...
describe('crashReporter module', () => {
  ...
  originalTempDirectory = app.getPath('temp')
    ...
    after(() => {app.setPath('temp', originalTempDirectory)})
      ...
      it('should send minidump when renderer crashes', function (done) {
        ...
        it('should send minidump when node processes crash', function(done) {
          ...
          if (process.env.APPVEYOR === 'True') {
            ...
            stopServer = startServer({
              callback(port) {
                const crashesDir = path.join(
                    app.getPath('temp'),
                    `${
                        process.platform === 'win32' ? 'Zombies' :
                                                       app.getName()} Crashes`)
                const version = app.getVersion()
                const crashPath = path.join(fixtures, 'module', 'crash.js')

                if (process.platform === 'win32') {
                  const crashServiceProcess = childProcess.spawn(
                      process.execPath,
                      [
                      ...
                      {
                        env: {ELECTRON_INTERNAL_CRASH_SERVICE: 1},
                        detached: true
                      })
                ...
                childProcess.fork(
                    crashPath, [port, version, crashesDir], {silent: true})
              },
              processType: 'browser',
              done: done
            })
        ...
        it('should not send minidump if uploadToServer is false',
           function(done) {
             this.timeout(180000)
          ...
          fs.readdir(crashesDir, (err, files) => {
                if (err) {
                  return const dumps = files.filter(
                             (file) => /\.dmp$/.test(file) &&
                                 !existingDumpFiles.has(file))
                  if (!dumps.length) {
                    return
                    ...
                    // dump file should not be deleted when not uploading, so we
                    // wait 1s and assert it still exists in `testDone`
        ...
        it('should send minidump with updated extra parameters',
           function(done) {
             // TODO(alexeykuzmin): Skip the test instead of marking it as
             // passed.
             if (process.env.APPVEYOR === 'True') {
               return done()
  ...
  const expectedName =
      (process.platform === 'darwin') ? 'Electron Test' : 'Zombies'
      ...
      assert.doesNotThrow(() => {
        ...
        describe(
            ...
            () => {it('correctly returns the directory', () => {
              ...
              else {dir = `${app.getPath('temp')}/Electron Test Crashes`} assert
                  .equal(crashesDir, dir)
            })})
