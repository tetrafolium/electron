...
describe('protocol module', () => {
  ...
  afterEach(
      (done) => {protocol.unregisterProtocol(
          protocolName,
            ...
            try {
              ...
              // Ignore error
                            ...
                            .registerStringProtocol(
                                protocolName, doubleHandler, (error) => {
                                  if (error) {
                                    return done(error)
  ...
  describe('protocol.registerStringProtocol', () => {
    ...
    it('sends object as response', (done) => {
                const handler =
                    (request,
                     callback) => {
                      callback({data: text, mimeType: 'text/html'})
                    } protocol
                                      .registerStringProtocol(
                                          protocolName, handler, (error) => {
                                            if (error) {
  ...
  describe('protocol.registerBufferProtocol', () => {
    ...
    it('sends object as response', (done) => {
                        const handler =
                            (request,
                             callback) => {
                              callback({data: buffer, mimeType: 'text/html'})
                            } protocol
  ...
  describe('protocol.registerFileProtocol', () => {
                          const filePath = path.join(
                              __dirname, 'fixtures', 'asar', 'a.asar', 'file1')
                          const fileContent =
                              require('fs').readFileSync(filePath)
                          const normalPath = path.join(
                              __dirname, 'fixtures', 'pages', 'a.html')
                          const normalContent =
                              require('fs').readFileSync(normalPath)
    ...
    it('fails when sending unexist-file', (done) => {
                                    const fakeFilePath = path.join(
                                        __dirname, 'fixtures', 'asar', 'a.asar',
                                        'not-exist')
                                    const handler = (request, callback) =>
                                    ...
                                    protocol.registerFileProtocol(
                                        protocolName, handler, (error) => {
                                          if (error) {
                                            ...
                                            $.ajax({
                                              url:
                                                  protocolName + '://fake-host',
                                              cache: false,
                                              success: () => done(
                                                  'request succeeded but it should not'),
                                              error: (xhr, errorType) => {
                                                assert.equal(errorType, 'error')
                                                done()
                                              }
  ...
  describe('protocol.registerStandardSchemes', () => {
    ...
    it('can have fetch working in it', (done) => {
                                                                                    const content =
                                                                                        '<html><script>fetch("http://github.com")</script></html>'
                                                                                    const handler =
                                                                                        (request,
                                                                                         callback) => callback({
                                                                                          data:
