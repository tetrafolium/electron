...
describe('session module', () => {
  ...
  describe('ses.cookies', () => {
          ...
          w.webContents.session.cookies.get({url}, (error, list) => {
            if (error) {
              ...
              for (let i = 0; i < list.length; i++) {
                const cookie = list[i] if (cookie.name === '0') {
                  if (cookie.value === '0') {
                    return done()
                  } else {
                    return done(
                        `cookie value is ${cookie.value} while expecting 0`)
                  }
                }
              }
            }
    ...
    it('calls back with an error when setting a cookie with missing required fields', (done) => {
      session.defaultSession.cookies.set({
        ...
        it('should over-write the existent cookie',
           (done) => {session.defaultSession.cookies.set(
               {url, name: '1', value: '1'}, (error) => {
                 if (error)
                   return done(error)
                   session.defaultSession.cookies.get({url}, (error, list) => {
                     if (error) {
                       return done(error)
                       for (let i = 0; i < list.length; i++) {
                         const cookie = list[i] if (cookie.name === '1') {
                           if (cookie.value === '1') {
                             return done()
                           } else {
                             return done(`cookie value is ${
                                 cookie.value} while expecting 1`)
                           }
                         }
                       }
                     }
  ...
  const downloadServer = http.createServer((req, res) => {
    ...
    webview.addEventListener(
        'did-finish-load', () => {webview.downloadURL(`${url}:${port}/`)})
        ipcRenderer.once('download-done', (event, state, url,
                                                    mimeType, receivedBytes,
                                                    totalBytes, disposition,
                                                    filename, savePath) => {
  ...
  beforeEach((done) => {
    if (w != null)
      w.destroy()
      w = new BrowserWindow(
          {show: false, webPreferences: {partition: partitionName}})
      partitionProtocol.registerStringProtocol(
          protocolName, handler,
          (error) => {done(error != null ? error : undefined)})
  })
      ...
      after((done) => {
        protocol.unregisterProtocol(scheme, () => {
          closeWindow(w).then(() => {
            ...
            const postData = JSON.stringify({type: 'blob', value: 'hello'})
            const content = `<html>
                       <script>
  ...
  describe('ses.setCertificateVerifyProc(callback)', () => {
    ...
    assert.equal(
        certificate.issuerCert.issuerCert.subject.commonName, 'Root CA')
  ...
  const options = {
    ...
    const rangeServer = http.createServer((req, res) => {
      let options = {root: fixtures} send(req, req.url, options)
    ...
    ipcRenderer.sendSync('set-download-option', true, false, downloadFilePath)
      ...
      const callback =
          (event, state, url, mimeType, receivedBytes, totalBytes, disposition,
           filename, savePath, urlChain, lastModifiedTime, eTag) => {
            if (state === 'cancelled') {
              const options = {
                path: savePath,
                urlChain: urlChain,
                ...
                eTag: eTag
                                  ...
                                  .sendSync(
                                      'set-download-option', false, false,
                                      downloadFilePath)
              w.webContents.session.createInterruptedDownload(options)
            } else {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (!credentials || credentials.name !== 'test' ||
  ...
  ses.setPermissionRequestHandler(() => {ses.setPermissionRequestHandler(null)})
