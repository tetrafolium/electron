...
describe('ipc renderer module', () => {
  ...
  describe('ipc.sender.send', () => {
    ...
    it('can send Electron API objects', (done) => {
      ...
      ipcRenderer.once('message', function(event, value) {
        assert.deepEqual(
            value.browserWindowOptions, webContents.browserWindowOptions);
        done();
      });
    ...
    it('does not crash on external objects (regression)', (done) => {
      ...
      const stream =
          request.agent.sockets['127.0.0.1:5000:'][0]._handle._externalStream;
      request.on('error', () => {});
      ipcRenderer.once(
          'message', function(event, requestValue, externalStreamValue) {
    ...
    it('can send objects that both reference the same object', (done) => {
      ...
      ipcRenderer.once(
          'message',
          function(event, arrayValue, fooValue, barValue, childValue) {
  ...
  describe('ipcRenderer.sendTo', () => {
    ...
    it('sends message to WebContents', (done) => {
      ...
      contents.loadURL(
          `file://${path.join(fixtures, 'pages', 'ping-pong.html')}`);
    });
  });

  describe('remote listeners', () => {
    it('detaches listeners subscribed to destroyed renderers, and shows a warning',
       (done) => {
         ...
         w.webContents.once('did-finish-load', () => {
           w.webContents.once('did-finish-load', () => {
             ...
             const results = ipcRenderer.sendSync(
                 'try-emit-web-contents-event', w.webContents.id,
                 'remote-handler');
         ...
         w.loadURL(`file://${
             path.join(fixtures, 'api', 'remote-event-handler.html')}`);
       });
  });
