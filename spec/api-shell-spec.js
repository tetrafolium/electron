...
describe('shell module', () => {
  ...
  describe('shell.readShortcutLink(shortcutPath)', () => {
    ...
    it('reads all properties of a shortcut', () => {
      const shortcut =
          shell.readShortcutLink(path.join(fixtures, 'assets', 'shortcut.lnk'));
  ...
  describe(
      'shell.writeShortcutLink(shortcutPath[, operation], options)', () => {
        ...
        it('writes the shortcut', () => {
          assert.equal(
              shell.writeShortcutLink(tmpShortcut, {target: 'C:\\'}), true);
          assert.equal(fs.existsSync(tmpShortcut), true);
        });

        it('correctly sets the fields', () => {
          assert.equal(
              shell.writeShortcutLink(tmpShortcut, shortcutOptions), true);
          assert.deepEqual(
              shell.readShortcutLink(tmpShortcut), shortcutOptions);
        });
        ...
        it('updates the shortcut', () => {
          ...
          assert.equal(
              shell.writeShortcutLink(tmpShortcut, 'update', change), true);
          assert.deepEqual(
              shell.readShortcutLink(tmpShortcut),
              Object.assign(shortcutOptions, change));
        });
        ...
        it('replaces the shortcut', () => {
          ...
          assert.equal(
              shell.writeShortcutLink(tmpShortcut, 'replace', change), true);
