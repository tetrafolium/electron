...
describe('electron module', () => {
  ...
  beforeEach(
      () => {
          window = new BrowserWindow({show: false, width: 400, height: 400})})

  afterEach(() => {return closeWindow(window).then(() => {window = null})})
...
describe('app module', () => {
  ...
  describe('app.exit(exitCode)', () => {
    ...
    describe(
        ...
        () => {it('prevents the second launch of app', function(done) {
          ...
          first.once('exit', (code) => {assert.equal(code, 0)})
  ...
  after(
    ...
    it('returns true', () => {assert.equal(returnValue, true)})
    ...
    it('returns false', () => {assert.equal(returnValue, false)})
  ...
  before(function() {
    ...
    assert.deepEqual(app.getLoginItemSettings(), {
      ...
      openAsHidden: process.platform === 'darwin' &&
          !process.mas,  // Only available on macOS
      wasOpenedAtLogin: false,
      wasOpenedAsHidden: false,
      restoreState: false
    })
    ...
    it('allows you to pass a custom executable and arguments', function () {
      ...
      app.setLoginItemSettings(
          {openAtLogin: true, path: updateExe, args: processStartArgs})

      assert.equal(app.getLoginItemSettings().openAtLogin, false)
      assert.equal(
          app.getLoginItemSettings({path: updateExe, args: processStartArgs})
              .openAtLogin,
          true)
    })
  })
    ...
    describe(
        'isAccessibilitySupportEnabled API',
        () => {
            it('returns whether the Chrome has accessibility APIs enabled',
               () => {assert.equal(
                   typeof app.isAccessibilitySupportEnabled(), 'boolean')})})
  ...
  beforeEach(
      () => {
          w = new BrowserWindow(
              {show: false, webPreferences: {partition: 'empty-certificate'}})})
  ...
  before(function() {
    if (process.platform !== 'win32') {
      ...
      classesKey = new Winreg({hive: Winreg.HKCU, key: '\\Software\\Classes\\'})
    }
  })
  ...
  after(function(done) {
    if (process.platform !== 'win32') {
      ...
      const protocolKey = new Winreg(
          {hive: Winreg.HKCU, key: `\\Software\\Classes\\${protocol}`})
    ...
    assert.equal(
        app.isDefaultProtocolClient(protocol, updateExe, processStartArgs),
        true)
      assert.equal(app.isDefaultProtocolClient(protocol), false)
    })
    ...
    const protocolKey =
        new Winreg({hive: Winreg.HKCU, key: `\\Software\\Classes\\${protocol}`})
  ...
  const sizes =
      {small: 16, normal: 32, large: process.platform === 'win32' ? 32 : 48}
  ...
  beforeEach(function() {
    ...
    assert.equal(size.width, sizes.small)
          ...
          it('fetches a normal icon',
             (done) => {app.getFileIcon(
                 iconPath, {size: 'normal'}, function(err, icon) {
                   const size = icon.getSize()
                   assert.equal(err, null)
                   assert.equal(size.height, sizes.normal)
                   assert.equal(size.width, sizes.normal)
      ...
      describe(
          ...
          () => {it(
              ...
              () => {
                ...
                assert.ok(
                    appMetrics.length > 0, 'App memory info object is not > 0')
                const types =
                    [] for (const {memory, pid, type, cpu} of appMetrics) {
                  assert.ok(
                      memory.workingSetSize > 0, 'working set size is not > 0')
  ...
  let server = null
  const socketPath = process.platform === 'win32' ?
      '\\\\.\\pipe\\electron-mixed-sandbox' :
      '/tmp/electron-mixed-sandbox'
  ...
  afterEach((done) => {
    ...
    describe('when the app is launched with --enable-mixed-sandbox', () => {
      it('adds --enable-sandbox to render processes created with sandbox: true', (done) => {
        const appPath = path.join(__dirname, 'fixtures', 'api', 'mixed-sandbox-app')
    appProcess = ChildProcess.spawn(
        remote.process.execPath, [appPath, '--enable-mixed-sandbox'])
