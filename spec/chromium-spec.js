...
describe('chromium feature', () => {
  ...
  describe('heap snapshot', () => {it('does not crash', function() {
  ...
  server.close()
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        describe(
            ...
            () => {it(
                'calls its callbacks',
                (done) => {navigator.webkitGetUserMedia(
                    {audio: true, video: false}, () => done(), () => done())})})

  describe('navigator.mediaDevices', () => {
    if (isCI) {
      return

          it('can return labels of enumerated devices',
             (done) => {navigator.mediaDevices.enumerateDevices()
                            .then((devices) => {
                              const labels =
                              ...
                              if (labelFound) {
                                ...
                                done(new Error(`No device labels found: ${
    ...
    w.webContents.on('ipc-message', (event, args) => {
      if (args[0] === 'deviceIds') {
        ...
        if (deviceIds.length === 2) {
          ...
          ses.clearStorageData(options, () => {w.webContents.reload()})
        }
      }
    })
  ...
  describe('navigator.serviceWorker', () => {
    ...
    it('should register for intercepted file scheme', (done) => {
      ...
      customSession.protocol.interceptBufferProtocol('file', (request, callback) => {
        ...
        w = new BrowserWindow(
            {show: false, webPreferences: {session: customSession}})
        w.webContents.on('ipc-message', (event, args) => {
          if (args[0] === 'reload') {
            w.webContents.reload()
          } else if (args[0] === 'error') {
            ...
            customSession.clearStorageData(
                ...
                () => {customSession.protocol.uninterceptProtocol(
                    'file', (error) => done(error))})
          }
        })
        w.loadURL(`file://${fixtures}/pages/service-worker/index.html`)
    })
  ...
  assert.equal(b.constructor.name, 'BrowserWindowProxy')
    ...
    listener = (event) => {
      ...
      const width = ref1[0] const height = ref1[1]
    ...
    const windowUrl = require('url').format({
      ...
      protocol: 'file',
      query: {p: `${fixtures}/pages/window-opener-node.html`},
      slashes: true
    })
    ...
    it('disables JavaScript when it is disabled on the parent window', (done) => {
      ...
      b.close()
            ...
            contents.sendInputEvent(
                {type: 'mouseDown', clickCount: 1, x: 1, y: 1})
    ...
    const windowUrl = require('url').format({
      ...
      protocol: 'file',
      query: {p: `${fixtures}/pages/window-opener-webview.html`},
      slashes: true
    })
    ...
    it('does not override child options', (done) => {
      let b
    const size = {width: 350, height: 450} listener = (event) => {
      ...
      assert.deepEqual(options.foo, {
        ...
        it('defines a window.location getter', (done) => {
          ...
          if (process.platform === 'win32') {
            targetURL =
                `file:///${fixtures.replace(/\\/g, '/')}/pages/base-page.html`
          } else {
    ...
    it('does not throw an exception when the features include webPreferences', () => {
      let b
    assert.doesNotThrow(() => {b = window.open('', '', 'webPreferences=')})
      b.close()
    })
  })
  ...
  before(
      (done) => {protocol.registerFileProtocol(
          scheme,
          (request, callback) => {
              callback(`${fixtures}/pages/window-opener-location.html`)},
          (error) => done(error))})

  after(() => {protocol.unregisterProtocol(scheme)})
  ...
  before(
      (done) => {protocol.registerFileProtocol(
          scheme, (request, callback) => {callback(srcPath)},
          (error) => done(error))})
  ...
  app.once(
      ...
      (event, {webContents}) => {webContents.once(
          'did-finish-load', () => {b.postMessage('testing', '*')})})
    ...
    it('supports windows opened from a <webview>', (done) => {
      ...
      assert.equal(e.message, 'message')
        ...
        webview.src = url.format({
          ...
          protocol: 'file',
          query: {p: `${fixtures}/pages/window-opener-postMessage.html`},
          slashes: true
        })
      document.body.appendChild(webview)
    })
    ...
    let server
      ...
      beforeEach((done) => {
        server = http.createServer((req, res) => {
          res.writeHead(200)
      const filePath =
          path.join(fixtures, 'pages', 'window-opener-targetOrigin.html')
  ...
  const message = 'ping'
    ...
    it('Worker has node integration with nodeIntegrationInWorker', (done) => {
      ...
      webview.remove()
        ...
        webview.setAttribute(
            'webpreferences', 'nodeIntegration, nodeIntegrationInWorker')
      document.body.appendChild(webview)
    })
    ...
    it('SharedWorker has node integration with nodeIntegrationInWorker', (done) => {
      let webview = new WebView()
    webview.addEventListener('console-message', (e) => {console.log(e)})
      webview.addEventListener('ipc-message', (e) => {
        assert.equal(e.channel, 'object function object function')
      webview.remove()
        done()
  ...
  beforeEach(() => {iframe = document.createElement('iframe')})
  ...
  describe('storage', () => {
          ...
          (request, callback) => {
            ...
            switch (parsedUrl.pathname) {
              case '/localStorage':
                ...
                break case '/sessionStorage': filename = 'session_storage.html';
                break case '/WebSQL': filename = 'web_sql.html';
                break case '/indexedDB': filename = 'indexed_db.html';
                break case '/cookie': filename = 'cookie.html';
    ...
    after((done) => {protocol.unregisterProtocol(protocolName, () => done())})
  ...
  const pdfSourceWithParams = url.format({
    ...
    webFrame.registerURLSchemeAsPrivileged('file', {
      ...
      assert.notEqual(document.title, 'cat.pdf')
        ...
        it('opens when loading a pdf resource in a iframe',
           (done) => {testPDFIsLoadedInSubFrame(
               'pdf-in-iframe.html', 'preload-pdf-loaded-in-subframe.js',
               done)})
