...
describe('ipc main module', () => {
  ...
  describe('ipc.sendSync', () => {
    ...
    it('does not crash when reply is not sent and browser is destroyed',
       (done) => {
         ...
         w.loadURL(
             `file://${path.join(fixtures, 'api', 'send-sync-message.html')}`);
       });
  ...
  describe('remote objects registry', () => {
    it('does not dereference until the render view is deleted (regression)',
       (done) => {
         w = new BrowserWindow({show: false});
         ...
         ipcMain.once('error-message', (event, message) => {
           assert(
               message.startsWith(
                   'Cannot call function \'getURL\' on missing remote object'),
               message);
           done();
         });

         w.loadURL(`file://${
             path.join(fixtures, 'api', 'render-view-deleted.html')}`);
       });
  });
});
