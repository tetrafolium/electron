...
const {app, session, getGuestWebContents, ipcMain, BrowserWindow, webContents} =
    remote
...
describe('<webview> tag', function () {
  ...
  beforeEach(() => {webview = new WebView()})
  ...
  const src = `<script src='file://${
      jqueryPath}'></script> <script>console.log('ok');</script>`
  ...
  describe('webpreferences attribute', () => {
    ...
    it('can disables web security and enable nodeintegration', (done) => {
      const jqueryPath = path.join(__dirname, '/static/jquery-2.0.3.min.js')
    const src = `<script src='file://${
        jqueryPath}'></script> <script>console.log('ok '+(typeof require));</script>`
    const encoded = btoa(unescape(encodeURIComponent(src)))
    const listener = (e) => {
  ...
  describe('page-favicon-updated event', () => {
    it('emits when favicon urls are received', (done) => {
      ...
      else {assert.equal(e.favicons[0], 'file:///favicon.png')} done()
      })
  webview.src = `file://${fixtures}/pages/a.html`
      document.body.appendChild(webview)
    })
  ...
  p = p.replace(/\\/g, '/')
  const pageUrl = url.format({protocol: 'file', slashes: true, pathname: p})
  ...
  const pageUrl = url.format({protocol: 'file', slashes: true, pathname: p})
    ...
    it('emits when window.history.replaceState is called', (done) => {
      webview.addEventListener('did-navigate-in-page', (e) => {
        ...
        webview.src = `file://${
            fixtures}/pages/webview-did-navigate-in-page-with-history.html`
      document.body.appendChild(webview)
    })
  ...
  describe('basic auth', () => {
    ...
    const server = http.createServer((req, res) => {
      ...
      else {res.end('failed')} server.close()
    })
  ...
  assert.deepEqual(e.args, ['C', 'KeyC', 67, true, false])
        ...
        webview.addEventListener(
            ...
            () => {webview.sendInputEvent(
                {type: 'keyup', keyCode: 'c', modifiers: ['shift']})})
    ...
    assert.deepEqual(e.args, [10, 20, false, true])
        ...
        webview.addEventListener(
            'dom-ready',
            () => {webview.sendInputEvent(
                {type: 'mouseup', modifiers: ['ctrl'], x: 10, y: 20})})
  ...
  webview.addEventListener('media-started-playing', () => {audioPlayed = true})
  ...
  describe('found-in-page event', () => {
    it('emits when a request is made', (done) => {
      let requestId = null
  let activeMatchOrdinal = [] const listener = (e) => {
  ...
  describe('permission-request event', () => {
      function setUpRequestHandler(webview, requestedPermission, completed) {
        ...
        } session.fromPartition(webview.partition)
  ...
  describe('did-get-response-details event', () => {
    it('emits for the page and its resources', (done) => {
      ...
      assert(!!expectedType, `Unexpected response details for ${event.newURL}`)
        ...
        webview.src = `file://${
            path.join(fixtures, 'pages', 'did-get-response-details.html')}`
      document.body.appendChild(webview)
    })
  })
  ...
  describe('will-attach-webview event', () => {
    ...
    webview.addEventListener('destroyed', () => {done()})
    webview.src = `file://${fixtures}/pages/c.html`
      document.body.appendChild(webview)
    })
  ...
  describe('guestinstance attribute', () => {
    ...
    it('setting the attribute on an existing webview moves the contents', (done) => {
      const load1Listener = () => {
          ...
          const webview2 = new WebView()
          const load2Listener = () => {
            webview2.removeEventListener(
                'did-finish-load', load2Listener, false)
    ...
    it('does not destroy the webContents when hiding/showing the webview (regression)', (done) => {
      webview.addEventListener('dom-ready', function domReadyListener () {
          ...
          // Wait for event directly since attach happens asynchronously over
          // IPC
    ...
    it('does not reload the webContents when hiding/showing the webview (regression)', (done) => {
      webview.addEventListener('dom-ready', function domReadyListener () {
          ...
          // Wait for event directly since attach happens asynchronously over
          // IPC
          webview.addEventListener('did-attach', () => {done()})
    ...
    w.loadURL(`file://${fixtures}/pages/webview-guest-resize.html`)
      ...
      w.webContents.once('did-finish-load', () => {
        ...
        assert.equal(height, CONTENT_SIZE)
            ...
            Promise.all([elementResizePromise, guestResizePromise])
                .then(() => done())
      ...
      w.webContents.once('did-finish-load', () => {
        ...
        const noGuestResizePromise = new Promise(resolve => {
          ...
          setTimeout(() => {
            ...
            Promise.all([elementResizePromise, noGuestResizePromise])
                .then(() => done())
        ...
        assert.equal(height, GUEST_HEIGHT)
          ...
          for (const wc of webContents.getAllWebContents()) {
            if (wc.hostWebContents &&
              ...
              wc.setSize({normal: {width: GUEST_WIDTH, height: GUEST_HEIGHT}})
            }
          }
            })
    })
  ...
  const webviewSession = session.fromPartition('webview-temp')
    ...
    it('maintains zoom level on navigation', (done) => {
            w = new BrowserWindow(
                {show: false, webPreferences: {zoomFactor: 1.2}})
            ipcMain.on(
                'webview-zoom-level',
                (event, zoomLevel, zoomFactor, newHost, final) => {
                  if (!newHost) {
                  ...
                  w.loadURL(
                      `file://${fixtures}/pages/webview-custom-zoom-level.html`)
                })
    ...
    ipcMain.on(
        'webview-zoom-in-page', (event, zoomLevel, zoomFactor, final) => {
          assert.equal(zoomFactor, 1.44)
          assert.equal(zoomLevel, 2.0)
          if (final) {
            done()
  ...
  webview.setAttribute('nodeintegration', 'on')
    ...
    it('opens window of about:blank with cross-scripting enabled', (done) => {
      ipcMain.once('answer', (event, content) => {
        ...
        webview.src = `file://${
            path.join(fixtures, 'api', 'native-window-open-blank.html')}`
      document.body.appendChild(webview)
    })
    ...
    it('opens window of same domain with cross-scripting enabled', (done) => {
      ipcMain.once('answer', (event, content) => {
        ...
        webview.src = `file://${
            path.join(fixtures, 'api', 'native-window-open-file.html')}`
      document.body.appendChild(webview)
    })
    ...
    it('returns null from window.open when allowpopups is not set', (done) => {
      ...
      ipcMain.once('answer', (event, {windowOpenReturnedNull}) => {
        ...
        webview.src = `file://${
            path.join(
                fixtures, 'api', 'native-window-open-no-allowpopups.html')}`
      document.body.appendChild(webview)
    })
    ...
    it('blocks accessing cross-origin frames', (done) => {
      ipcMain.once('answer', (event, content) => {
        ...
        webview.src = `file://${
            path.join(fixtures, 'api', 'native-window-open-cross-origin.html')}`
      document.body.appendChild(webview)
    })
