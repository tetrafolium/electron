...
describe("systemPreferences module", () => {
  ...
  describe("systemPreferences.getColor(id)", () => {
    ...
    it("returns a hex RGB color string", () => {
      assert.equal(
        /^#[0-9A-F]{6}$/i.test(systemPreferences.getColor("window")),
        true
      );
    });
  });
  ...
  describe("systemPreferences.getUserDefault(key, type)", () => {
    ...
    it("returns values for known user defaults", () => {
      ...
      const languages = systemPreferences.getUserDefault(
        "AppleLanguages",
        "array"
      );
    ...
    it("returns values for unknown user defaults", () => {
      assert.equal(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "boolean"),
        false
      );
      assert.equal(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "integer"),
        0
      );
      assert.equal(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "float"),
        0
      );
      assert.equal(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "double"),
        0
      );
      assert.equal(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "string"),
        ""
      );
      assert.equal(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "url"),
        ""
      );
      assert.equal(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "badtype"),
        undefined
      );
      assert.deepEqual(
        systemPreferences.getUserDefault("UserDefaultDoesNotExist", "array"),
        []
      );
      assert.deepEqual(
        systemPreferences.getUserDefault(
          "UserDefaultDoesNotExist",
          "dictionary"
        ),
        {}
      );
