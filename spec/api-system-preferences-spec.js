...
describe('systemPreferences module', () => {
  ...
  describe('systemPreferences.getColor(id)', () => {
    ...
    it('returns a hex RGB color string', () => {
      assert.equal(
          /^#[0-9A-F]{6}$/i.test(systemPreferences.getColor('window')), true);
    });
  });
  ...
  describe('systemPreferences.getUserDefault(key, type)', () => {
    ...
    it('returns values for known user defaults', () => {
      ...
      const languages =
          systemPreferences.getUserDefault('AppleLanguages', 'array');
    ...
    it('returns values for unknown user defaults', () => {
      assert.equal(
          systemPreferences.getUserDefault(
              'UserDefaultDoesNotExist', 'boolean'),
          false);
      assert.equal(
          systemPreferences.getUserDefault(
              'UserDefaultDoesNotExist', 'integer'),
          0);
      assert.equal(
      ...
      assert.equal(
          systemPreferences.getUserDefault(
              'UserDefaultDoesNotExist', 'badtype'),
          undefined);
      assert.deepEqual(
      ...
      assert.deepEqual(
          systemPreferences.getUserDefault(
              'UserDefaultDoesNotExist', 'dictionary'),
          {});
    });
  });
      ...
      for (const [type, value] of TEST_CASES) {
        assert.throws(() => {
          systemPreferences.setUserDefault(
              KEY, type, typeof value === 'string' ? {} : 'foo');
