...
describe('net module', () => {
  ...
  server.on('request', (request, response) => {
    switch (request.url) {
      case requestUrl:
        ...
        response.end()
        break default: handleUnexpectedURL(request, response)
    }
  })
  const urlRequest = net.request(`${server.url}${requestUrl}`)
    ...
    const urlRequest =
        net.request({method: 'POST', url: `${server.url}${requestUrl}`})
      urlRequest.on('response', (response) => {
        assert.equal(response.statusCode, 200)
      response.pause()
      response.on('data', (chunk) => {})
    ...
    const urlRequest = net.request(`${server.url}${requestUrl}`)
      ...
      assert.equal(response.statusCode, 200)
      response.pause()
      response.on('data', (chunk) => {expectedBodyData += chunk.toString()})
        response.on('end', () => {
    ...
    server.on('request', (request, response) => {
      ...
      switch (request.url) {
        case requestUrl:
          assert.equal(request.method, 'POST')
          request.on('data', (chunk) => {postedBodyData += chunk.toString()})
            request.on('end', () => {
    ...
    server.on('request', (request, response) => {
      switch (request.url) {
        case requestUrl:
          ...
          request.on('end', (chunk) => {response.end(chunk)})
          break default: handleUnexpectedURL(request, response)
      }
    })
    const urlRequest =
    ...
    let sentChunks = [] let receivedChunks = []
      urlRequest.on('response', (response) => {
        assert.equal(response.statusCode, 200)
      response.pause()
      response.on('data', (chunk) => {receivedChunks.push(chunk)})
        response.on('end', () => {
  ...
  describe('ClientRequest API', () => {
    ...
    const urlRequest =
        net.request({method: 'GET', url: `${server.url}${requestUrl}`})
      urlRequest.on('response', (response) => {
      ...
      response.pause()
        ...
        responseEndEventEmitted = true
          ...
          response.resume()
          response.on('error', (error) => {assert.ifError(error)})
        response.on('aborted', () => {
          assert.fail('response aborted')
        })
      })
        urlRequest.on('finish', () => {requestFinishEventEmitted = true})
        urlRequest.on('error', (error) => {assert.ifError(error)})
        urlRequest.on('abort', () => {assert.fail('request aborted')})
      urlRequest.on('close', () => {
    ...
    server.on('request', (request, response) => {
      switch (request.url) {
        case requestUrl:
          assert.equal(
              request.headers[customHeaderName.toLowerCase()],
              customHeaderValue)
          response.statusCode = 200
          response.statusMessage = 'OK'
          response.end()
    ...
    const urlRequest =
      ...
      response.on('end', () => {done()})
        ...
        assert.equal(urlRequest.getHeader(customHeaderName), customHeaderValue)
        assert.equal(
            urlRequest.getHeader(customHeaderName.toLowerCase()),
            customHeaderValue)
        urlRequest.write('')
        assert.equal(urlRequest.getHeader(customHeaderName), customHeaderValue)
        assert.equal(
    ...
    server.on('request', (request, response) => {
      switch (request.url) {
        case requestUrl:
          ...
          response.statusMessage = 'OK'
          response.end()
          break default: assert.equal(request.url, requestUrl)
      }
    })
    const urlRequest =
        net.request({method: 'GET', url: `${server.url}${requestUrl}`})
    ...
    const urlRequest =
      ...
      response.on('end', () => {done()})
        ...
        assert.equal(urlRequest.getHeader(customHeaderName), customHeaderValue)
        urlRequest.write('')
        assert.throws(() => {urlRequest.removeHeader(customHeaderName)})
        assert.equal(urlRequest.getHeader(customHeaderName), customHeaderValue)
      urlRequest.end()
    })
    ...
    server.on('request', (request, response) => {
      switch (request.url) {
        case requestUrl:
          assert.equal(
              request.headers[cookieHeaderName.toLowerCase()],
              cookieHeaderValue)
          response.statusCode = 200
          response.statusMessage = 'OK'
          response.end()
    ...
    customSession.cookies.set(
        {url: `${server.url}`, name: 'test', value: '11111'}, (error) => {
          if (error) {
            return done(error)
        ...
        response.on('end', () => {done()})
          ...
          assert.equal(
              urlRequest.getHeader(cookieHeaderName), cookieHeaderValue)
          urlRequest.end()
          })
        })
    ...
    it('should be able to abort an HTTP request before first write', (done) => {
      ...
      server.on('request', (request, response) => {
        ...
        urlRequest.on(
            'response',
            (response) => {assert.fail('Unexpected response event')})
        urlRequest.on('finish', () => {assert.fail('Unexpected finish event')})
        urlRequest.on('error', () => {assert.fail('Unexpected error event')})
        urlRequest.on('abort', () => {requestAbortEventEmitted = true})
      urlRequest.on('close', () => {
        requestCloseEventEmitted = true
    ...
    const urlRequest =
        net.request({method: 'GET', url: `${server.url}${requestUrl}`})
    urlRequest.on(
        'response', (response) => {assert.fail('Unexpected response event')})
    urlRequest.on('finish', () => {assert.fail('Unexpected finish event')})
    urlRequest.on('error', () => {assert.fail('Unexpected error event')})
    urlRequest.on('abort', () => {requestAbortEventEmitted = true})
      urlRequest.on('close', () => {
    ...
    const urlRequest =
      ...
      response.pause()
      response.on('data', (chunk) => {})
      response.on('end', () => {assert.fail('Unexpected end event')})
      response.resume()
      response.on('error', () => {assert.fail('Unexpected error event')})
      response.on('aborted', () => {responseAbortedEventEmitted = true})
        urlRequest.abort()
      })
        urlRequest.on('finish', () => {requestFinishEventEmitted = true})
        urlRequest.on('error', () => {assert.fail('Unexpected error event')})
      ...
      assert(
          requestReceivedByServer, 'request should be received by the server')
      ...
      assert(
          responseAbortedEventEmitted, 'response should emit "aborted" event')
    ...
    const urlRequest =
        net.request({method: 'GET', url: `${server.url}${requestUrl}`})
    urlRequest.on('response', () => {assert.fail('Unexpected response event')})
    urlRequest.on('finish', () => {requestFinishEventEmitted = true})
    urlRequest.on('error', () => {assert.fail('Unexpected error event')})
      urlRequest.on('abort', () => {
    ...
    session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
      if (details.url === `${server.url}${requestUrl}`) {
        ...
        callback({redirectURL: `${server.url}${redirectUrl}`})
      } else {
    ...
    const urlRequest = net.request(`${server.url}${requestUrl}`)
      ...
      response.on('data', (chunk) => {})
        ...
        assert(
            requestIsIntercepted,
            'The request should be intercepted by the webRequest module')
          done()
        })
        response.resume()
      })
    ...
    session.defaultSession.webRequest.onBeforeRequest(
        (details, callback) => {assert.fail(
            'Request should not be intercepted by the default session')})

    let customSession =
        session.fromPartition(customPartitionName, {cache: false})
    let requestIsIntercepted = false
    ...
    const urlRequest =
        net.request({url: `${server.url}${requestUrl}`, session: customSession})
      urlRequest.on('response', (response) => {
        assert.equal(response.statusCode, 200)
      response.pause()
      response.on('data', (chunk) => {})
    ...
    const urlRequest = net.request({url: `${server.url}${requestUrl}`})
      urlRequest.on('response', (response) => {
        assert.equal(response.statusCode, 200)
        done()
      })
    ...
    const urlRequest =
        net.request({url: `${server.url}${requestUrl}`, redirect: 'error'})
    urlRequest.on(
        'error',
        (error) => {assert.equal(
            error.message,
            'Request cannot follow redirect with the current redirect mode')})
    urlRequest.on('close', () => {done()})
      urlRequest.end()
    })
    ...
    const urlRequest =
        net.request({url: `${server.url}${requestUrl}`, redirect: 'manual'})
      urlRequest.on('response', (response) => {
        assert.equal(response.statusCode, 200)
    ...
    server.on('request', (request, response) => {
      switch (request.url) {
        case '/redirectChain':
          ...
          response.end()
          break case '/redirect/1': response.statusCode = '200'
          response.setHeader('Location', '/redirect/2')
          response.end()
          break case '/redirect/2': response.statusCode = '200'
          response.end()
          break default: handleUnexpectedURL(request, response)
      }
    })
    const urlRequest =
      ...
      response.pause()
      response.on('data', (chunk) => {})
      response.on('end', () => {urlRequest.abort()})
        response.resume()
      })
      urlRequest.on('close', () => {
      ...
      it('should throw if given an invalid session option', (done) => {
        ...
        try {
          const urlRequest =
              net.request({url: `${server.url}${requestUrl}`, session: 1})
    ...
    const urlRequest = net.request(
        {url: `${server.url}${requestUrl}`, partition: customPartitionName})
      urlRequest.on('response', (response) => {
        assert.equal(response.statusCode, 200)
      response.pause()
      response.on('data', (chunk) => {})
      ...
      it('should throw if given an invalid partition option', (done) => {
        ...
        try {
          const urlRequest =
              net.request({url: `${server.url}${requestUrl}`, partition: 1})
    ...
    let options = {
      port: serverUrl.port, hostname: '127.0.0.1', headers: {}
    ...
    server.on('request', (request, response) => {
      switch (request.url) {
        case nodeRequestUrl:
          response.write(bodyData)
          response.end()
          break case netRequestUrl: netRequestReceived = true
          let receivedBodyData = ''
          request.on('data', (chunk) => {receivedBodyData += chunk.toString()})
  ...
  const urlRequest =
      ...
      assert.deepEqual(
          headers[customHeaderName.toLowerCase()], [customHeaderValue])
    ...
    server.on('request', (request, response) => {
      switch (request.url) {
        case netRequestUrl:
          ...
          response.write(bodyData)
          response.end()
          break case nodeRequestUrl: let receivedBodyData = ''
          request.on('data', (chunk) => {receivedBodyData += chunk.toString()})
          request.on('end', (chunk) => {
            if (chunk) {
              receivedBodyData += chunk.toString()
    ...
    ipcRenderer.once('api-net-spec-done', () => {done()})
    ...
    const urlRequest =
      ...
      response.on('end', () => {})
      response.resume()
      response.on('error', () => {assert(!requestCloseEventEmitted)})
        response.on('aborted', () => {
          assert(!requestCloseEventEmitted)
        })
      })
        urlRequest.on('finish', () => {assert(!requestCloseEventEmitted)})
        urlRequest.on('error', () => {assert(!requestCloseEventEmitted)})
        urlRequest.on('abort', () => {assert(!requestCloseEventEmitted)})
      urlRequest.on('close', () => {
        requestCloseEventEmitted = true
