...
const skip =
    process.platform !== 'linux' || !process.env.DBUS_SYSTEM_BUS_ADDRESS;
...
(skip ? describe.skip : describe)('powerMonitor', () => {
  ...
  before(async () => {
    ...
    const getInterface = Promise.promisify(loginService.getInterface, {
      context: loginService,
    });
    logindMock = await getInterface(
        '/org/freedesktop/login1', 'org.freedesktop.DBus.Mock');
    getCalls = Promise.promisify(logindMock.GetCalls, {context: logindMock});
    emitSignal = Promise.promisify(logindMock.EmitSignal, {
      context: logindMock,
    });
  ...
  describe('when powerMonitor module is loaded', () => {
    ...
    describe('when PrepareForSleep(true) signal is sent by logind', () => {
      it('should emit "suspend" event', (done) => {
        ...
        emitSignal('org.freedesktop.login1.Manager', 'PrepareForSleep', 'b', [
          ['b', true],
        ]);
      });
