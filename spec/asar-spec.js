...
describe('asar package', function() {
  ...
  describe('node api', function() {
      ...
      it('does not leak fd', function() {
        ...
        while (readCalls <= 10000) {
          fs.readFileSync(path.join(
              process.resourcesPath, 'electron.asar', 'renderer', 'api',
              'ipc-renderer.js'));
          readCalls++;
        }
      });
    ...
    describe('fs.readFile', function() {
      ...
      it('reads a file from linked directory', function(done) {
        var p =
            path.join(fixtures, 'asar', 'a.asar', 'link2', 'link2', 'file1');
        fs.readFile(p, function(err, content) {
          assert.equal(err, null);
          assert.equal(String(content).trim(), 'file1');
          done();
    ...
    describe('fs.lstatSync', function() {
      ...
      it('returns information of a normal file', function() {
        ...
        ref2 = [
          ...
          'file2',
          'file3',
          path.join('dir1', 'file1'),
          path.join('link2', 'file1'),
        ];
      ...
      it('returns information of a linked file', function() {
        ...
        ref2 = [
          'link1',
          path.join('dir1', 'link1'),
          path.join('link2', 'link2'),
        ];
        for (j = 0, len = ref2.length; j < len; j++) {
          file = ref2[j];
          p = path.join(fixtures, 'asar', 'a.asar', file);
    ...
    describe('child_process.fork', function() {
      it('opens a normal js file', function(done) {
        var child =
            ChildProcess.fork(path.join(fixtures, 'asar', 'a.asar', 'ping.js'));
    ...
    describe('child_process.exec', function() {
      ...
      it('should not try to extract the command if there is a reference to a file inside an .asar',
         function(done) {
           ChildProcess.exec(
               'echo ' + echo + ' foo bar', function(error, stdout) {
                 assert.equal(error, null);
                 assert.equal(
                     stdout.toString().replace(/\r/g, ''), echo + ' foo bar\n');
                 done();
               });
         });
    });
    ...
    describe('process.noAsar', function() {
      ...
      it('is reset to its original value when execSync throws an error',
         function() {
           process.noAsar = false;
           assert.throws(function() {
    ...
    describe('process.env.ELECTRON_NO_ASAR', function() {
      it('disables asar support in forked processes', function(done) {
        const forked = ChildProcess.fork(
            path.join(__dirname, 'fixtures', 'module', 'no-asar.js'), [], {
              env: {
  ...
  describe('original-fs module', function() {
    ...
    it('is available in forked scripts', function(done) {
      var child =
          ChildProcess.fork(path.join(fixtures, 'module', 'original-fs.js'));
      child.on('message', function(msg) {
