...
async function createReleaseBranch() {
  ...
  let checkoutDetails =
      await GitProcess.exec(['checkout', '-b', 'release'], gitDir);
  if (checkoutDetails.exitCode === 0) {
    ...
    if (error === GitError.BranchAlreadyExists) {
      ...
      console.log(
          `${fail} Error creating release branch: ` +
...
async function getReleaseNotes(currentBranch) {
  ...
  let commitComparison =
      await github.repos.compareCommits(githubOpts).catch((err) => {
        console.log(
...
async function pushRelease() {
  ...
  if (pushDetails.exitCode === 0) {
    ...
    console.log(
        `${fail} Error pushing the release branch: ` +
        `${pushDetails.stderr}`);
    process.exit(1);
  }
}
