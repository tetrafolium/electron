...
if (!versionType && !args.notesOnly) {
  console.log(
    `Usage: prepare-release versionType [major | minor | patch | beta]` +
      ` (--stable) (--notesOnly)`
  );
  process.exit(1);
}
...
async function createReleaseBranch() {
  ...
  let checkoutDetails = await GitProcess.exec(
    ["checkout", "-b", "release"],
    gitDir
  );
  if (checkoutDetails.exitCode === 0) {
    ...
    if (error === GitError.BranchAlreadyExists) {
      console.log(
        `${fail} Release branch already exists, aborting prepare ` +
          `release process.`
      );
    } else {
      console.log(
        `${fail} Error creating release branch: ` + `${checkoutDetails.stderr}`
      );
    }
    process.exit(1);
  }
...
async function getCurrentBranch(gitDir) {
  ...
  if (branchDetails.exitCode === 0) {
    ...
    console.log(
      `${pass} Successfully determined current git branch is ` +
        `${currentBranch}`
    );
    ...
    console.log(
      `${fail} Could not get details for the current branch,
      error was ${branchDetails.stderr}`,
      error
    );
    process.exit(1);
  }
...
async function getReleaseNotes(currentBranch) {
  ...
  let commitComparison = await github.repos
    .compareCommits(githubOpts)
    .catch((err) => {
      console.log(
        `{$fail} Error checking for commits from ${pkg.version} to ` +
          `${currentBranch}`,
        err
      );
      process.exit(1);
...
async function createRelease(branchToTarget, isBeta) {
  ...
  let releases = await github.repos.getReleases(githubOpts).catch((err) => {
  ...
  await github.repos.createRelease(githubOpts).catch((err) => {
...
async function pushRelease() {
  ...
  if (pushDetails.exitCode === 0) {
    console.log(
      `${pass} Successfully pushed the release branch.  Wait for ` +
        `release builds to finish before running "npm run release".`
    );
  } else {
    console.log(
      `${fail} Error pushing the release branch: ` + `${pushDetails.stderr}`
    );
    process.exit(1);
  }
}
