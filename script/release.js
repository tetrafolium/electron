...
assert(
    process.env.ELECTRON_GITHUB_TOKEN,
    'ELECTRON_GITHUB_TOKEN not found in environment')

const github = new GitHub({followRedirects: false})
github.authenticate({type: 'token', token: process.env.ELECTRON_GITHUB_TOKEN})
const gitDir = path.resolve(__dirname, '..')

async function getDraftRelease(version, skipValidation) {
  let releaseInfo =
      await github.repos.getReleases({owner: 'electron', repo: 'electron'})
  let drafts
  let versionToCheck
  if (version) {
    drafts = releaseInfo.data.filter(release => release.tag_name === version)
    versionToCheck = version
  }
  else {
    drafts = releaseInfo.data.filter(release => release.draft)
    versionToCheck = pkgVersion
  }

  const draft = drafts[0] if (!skipValidation) {
    failureCount = 0
    check(drafts.length === 1, 'one draft exists', true)
    check(
        draft.tag_name === versionToCheck,
        `draft release version matches local package.json (${versionToCheck})`)
    if (versionToCheck.indexOf('beta')) {
      check(draft.prerelease, 'draft is a prerelease')
    }
    check(
        draft.body.length > 50 && !draft.body.includes('(placeholder)'),
...
async function validateReleaseAssets(release) {
  ...
  const downloadUrls =
      release.assets.map(asset => asset.browser_download_url).sort()

  failureCount = 0
  requiredAssets.forEach(asset => {check(extantAssets.includes(asset), asset)})
  check(
      (failureCount === 0), `All required GitHub assets exist for release`,
      true)
  ...
  if (release.draft) {
    ...
    await verifyShasums(downloadUrls)
        .catch(err => {console.log(`${fail} error verifyingShasums`, err)})
  }
  const s3Urls =
      s3UrlsForVersion(release.tag_name) await verifyShasums(s3Urls, true)
}
...
function checkVersion() {
  console.log(
      `Verifying that app version matches package version ${pkgVersion}.`)
  let startScript = path.join(__dirname, 'start.py')
  let appVersion = runScript(startScript, ['--version']).trim()
  check(
      (pkgVersion.indexOf(appVersion) === 0),
      `App version ${appVersion} matches ` +
...
function runScript(scriptName, scriptArgs, cwd) {
  ...
  let scriptOptions = { encoding: 'UTF-8' } if (cwd) {
  ...
  if (existingAssets.length > 0) {
    console
        .log(`${
            ...
            await github.repos
        .deleteAsset(
            {owner: 'electron', repo: 'electron', id: existingAssets[0].id})
        .catch(
            err => {console.log(
                `${fail} Error deleting ${fileName} on GitHub:`, err)})
  }
...
function saveShaSumFile(checksums, fileName) {
  return new Promise((resolve, reject) => {temp.open(fileName, (err, info) => {
                       if (err) {
                         console.log(
                             `${fail} Could not create ${fileName} file`)
                         process.exit(1)
                       } else {
...
async function makeRelease(releaseToValidate) {
  if (releaseToValidate) {
    ...
    uploadNodeShasums()
    uploadIndexJson() await createReleaseShasums(draftRelease)
    // Fetch latest version of release before verifying
    draftRelease =
        await getDraftRelease(pkgVersion, true) await validateReleaseAssets(draftRelease) await publishRelease(draftRelease) await cleanupReleaseBranch()
    console.log(
...
async function makeTempDir() {
  return new Promise(
      (resolve, reject) => {temp.mkdir('electron-publish', (err, dirPath) => {
        if (err) {
          reject(err)
        } else {
          resolve(dirPath)
...
async function verifyAssets(release) {
  ...
  let assetDetails =
      await github.repos.getAsset(githubOpts) await downloadFiles(
          assetDetails.meta.location, downloadDir, false, asset.name)
    return asset.name
  })).catch(err => {
    ...
    let checkerOpts await validateChecksums({
      algorithm: 'sha256',
      filesToCheck,
...
function downloadFiles(urls, directory, quiet, targetName) {
  return new Promise((resolve, reject) => {
    let nuggetOpts = { dir: directory } if (quiet) {
...
async function verifyShasums(urls, isS3) {
  ...
  let filesToCheck = [] try {
    if (!isS3) {
      ...
      const s3VersionPath = `/atom-shell/dist/${pkgVersion}/` await Promise.all(
          urls.map(async (url) => {
            let currentUrl = new URL(url)
            ...
            if (s3VersionPathIdx === -1 || dirname === s3VersionPath) {
              if (s3VersionPathIdx !== -1 &&
                  filename.indexof('SHASUMS') === -1) {
              ...
              let subDirectory =
                  dirname.substr(s3VersionPathIdx + s3VersionPath.length)
...
async function validateChecksums(validationArgs) {
  console.log(
      `Validating checksums for files from ${validationArgs.fileSource} ` +
      `against ${validationArgs.shaSumFile}.`)
  let shaSumFilePath =
      path.join(validationArgs.fileDirectory, validationArgs.shaSumFile) {
    {
      {
        {
          {
                                                          ...
                                                          new sumchecker
                                                              ...
                                                              .catch(err => {
                                                                if (err instanceof
                                                                  ...
                                                                  console.error(
                                                                      `${fail} The checksum of ${
                                                                          err.filename} from ` +
                                                                      `${
                                                                          validationArgs
                                                                              .fileSource} did not match the shasum in ` +
                                                                      `${
                                                                          validationArgs
                                                                              .shaSumFile}`)
                                                                } else if (
                                                                    ...
                                                                    sumchecker
                                                                        .ChecksumParseError) {
                                                                  console.error(
                                                                      `${fail} The checksum file ${
                                                                          validationArgs
                                                                              .shaSumFile} ` +
                                                                          `from ${
                                                                              validationArgs
                                                                                  .fileSource} could not be parsed.`,
                                                                      err)
                                                                } else if (
                                                                    err instanceof
                                                                    sumchecker
                                                                  ...
                                                                  console.error(
                                                                      ...
                                                                      `${
                                                                          validationArgs
                                                                              .fileSource} was not in the shasum file ` +
                                                                      `${
                                                                          validationArgs
                                                                              .shaSumFile}.`)
                                                                } else {
                                                                  console.error(
                                                                      `${fail} Error matching files from ` +
                                                                          `${
                                                                              validationArgs
                                                                                  .fileSource} shasums in ${
                                                                              validationArgs
                                                                                  .shaSumFile}.`,
                                                                      err)
                                                                }
...
async function cleanupReleaseBranch() {
  ...
  let successMessage =
      `Successfully deleted local release branch.` await callGit(
          ['branch', '-D', 'release'], errorMessage, successMessage)
