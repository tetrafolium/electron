...
const branchToRelease = process.argv[2] const fail = '\u2717'.red
...
async function commitMerge() {
  ...
  let gitArgs = ['commit', '-m', `v${pkg.version}`] return callGit(
      gitArgs, errorMessage, successMessage) {}

  async function mergeReleaseIntoBranch(branchName) {
    console.log(`Merging release branch into ${branchName}.`)
    let mergeArgs = ['merge', 'release', '--squash'] let mergeDetails =
        await GitProcess.exec(mergeArgs, gitDir)
    if (mergeDetails.exitCode === 0) {
      ...
      if (error === GitError.MergeConflicts) {
        console.log(
            `${fail} Could not merge release branch into ${branchName} ` +
  ...
  async function pushBranch(branchName) {
    ...
    let pushArgs = ['push', 'origin', branchName] let errorMessage =
        `Could not push branch ${branchName} due to an error:`
  ...
  async function mergeRelease() {
    ...
    await commitMerge()
      ...
      if (pushSuccess) {
        console.log(
            `${pass} Success!!! ${branchToRelease} now has the latest release!`)
      }
    }
    else {
      console.log(`Trying rebase of ${branchToRelease} into release branch.`) await pull()
      ...
      if (rebaseResult) {
        ...
        if (pushResult) {
          console.log(
              `Rebase of ${branchToRelease} into release branch was ` +
