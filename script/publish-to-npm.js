...
const files = ['cli.js', 'index.js', 'install.js', 'package.json', 'README.md'];
...
new Promise((resolve, reject) => {
  temp.mkdir('electron-npm', (err, dirPath) => {
      ...
      files.forEach((name) => {
        fs.writeFileSync(
            ...
            fs.readFileSync(path.join(
                __dirname, '..', name === 'README.md' ? '' : 'npm', name)));
      });
    ...
    .then((releases) => {
      ...
      if (!release) {
        throw new Error(
            `cannot find release with tag v${rootPackageJson.version}`);
      }
      return release;
    })
    .then((release) => {
      const tsdAsset =
          release.assets.find((asset) => asset.name === 'electron.d.ts');
      if (!tsdAsset) {
        throw new Error(`cannot find electron.d.ts from v${
            rootPackageJson.version} release assets`);
      }
    ...
    .then(() => {
      ...
      const tarballPath = path.join(
          tempDir, `${rootPackageJson.name}-${rootPackageJson.version}.tgz`);
      return new Promise((resolve, reject) => {
        childProcess.execSync(`npm install ${tarballPath} --force --silent`, {
          env: Object.assign({}, process.env, {electron_config_cache: tempDir}),
          cwd: tempDir,
        });
        const checkVersion = childProcess.execSync(
            `${path.join(tempDir, 'node_modules', '.bin', 'electron')} -v`);
        assert.ok(
            `v${rootPackageJson.version}`.indexOf(
                checkVersion.toString().trim()) === 0,
            `Version is correct`);
        resolve(tarballPath);
      });
    })
