...
new Promise((resolve, reject) => {
  ...
  .then((dirPath) => {
    ...
    files.forEach((name) => {
      fs.writeFileSync(
        ...
        fs.readFileSync(
          path.join(__dirname, "..", name === "README.md" ? "" : "npm", name)
        )
      );
    });
  ...
  .then((release) => {
    const tsdAsset = release.assets.find(
      (asset) => asset.name === "electron.d.ts"
    );
    if (!tsdAsset) {
      throw new Error(
        `cannot find electron.d.ts from v${rootPackageJson.version} release assets`
      );
    }
