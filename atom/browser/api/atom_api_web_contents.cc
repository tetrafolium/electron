...
void WebContents::ShowAutofillPopup(content::RenderFrameHost* frame_host,
  ...
  if (relay) {
    relay->window->ShowAutofillPopup(frame_host, web_contents(), bounds, values,
                                     labels);
  }
}
...
bool WebContents::OnMessageReceived(const IPC::Message& message,
  ...
  IPC_BEGIN_MESSAGE_MAP_WITH_PARAM(NativeWindow, message, frame_host)
    IPC_MESSAGE_FORWARD(AtomAutofillFrameHostMsg_HidePopup, relay->window.get(),
                        NativeWindow::HideAutofillPopup)
    IPC_MESSAGE_UNHANDLED(handled = false)
  IPC_END_MESSAGE_MAP()
