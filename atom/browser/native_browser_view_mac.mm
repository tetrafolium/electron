...
void NativeBrowserViewMac::UpdateDraggableRegions(
  ...
  if (webViewHeight > superViewHeight) {
    webViewY = std::abs(webViewHeight - superViewHeight -
                        (std::abs(NSMinY([webView frame]))));
  } else {
  ...
  [dragRegion setFrame:NSMakeRect(0, 0, webViewWidth, webViewHeight)];
  ...
  for (auto iter = system_drag_exclude_areas.begin();
    ...
    [controlRegion
        setFrame:NSMakeRect(iter->x() - webViewX,
