...
- (NSRect)windowWillUseStandardFrame:(NSWindow*)window
  ...
  CGFloat page_width =
      static_cast<CGFloat>(web_contents->GetPreferredSize().width());
...
@interface AtomNSWindow : EventDispatchingWindow <QLPreviewPanelDataSource,
...
- (void)previewFileAtPath:(NSString*)path withName:(NSString*)fileName {
  ...
  [self setQuickLookItem:[[[AtomPreviewItem alloc] initWithURL:url
                                                         title:fileName]
                             autorelease]];
...
struct Converter<atom::NativeWindowMac::TitleBarStyle> {
  static bool FromV8(v8::Isolate* isolate,
    ...
    } else if (title_bar_style ==
                   "hidden-inset" ||  // TODO(kevinsawicki): Remove in 2.0,
                                      // deprecate before then with warnings
...
void NativeWindowMac::RenderViewHostChanged(content::RenderViewHost* old_host,
...
void NativeWindowMac::UpdateDraggableRegionViews(
  ...
  for (std::vector<gfx::Rect>::const_iterator iter =
       ...
       iter != system_drag_exclude_areas.end(); ++iter) {
    base::scoped_nsobject<NSView> controlRegion(
        [[ControlRegionView alloc] initWithFrame:NSZeroRect]);
    [controlRegion
        setFrame:NSMakeRect(iter->x(), webViewHeight - iter->bottom(),
