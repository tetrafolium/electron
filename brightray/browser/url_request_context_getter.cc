...
net::URLRequestContext* URLRequestContextGetter::GetURLRequestContext() {
  ...
  if (!url_request_context_.get()) {
    ...
    auto cookie_config = content::CookieStoreConfig(
        ...
        nullptr, delegate_->CreateCookieDelegate());
    ...
    if (command_line.HasSwitch(switches::kNoProxyServer)) {
      ...
      storage_->set_proxy_service(net::CreateProxyServiceUsingV8ProxyResolver(
    ...
    if (network_controller_handle_) {
      ...
      storage_->set_http_transaction_factory(
          base::WrapUnique(new net::HttpCache(http_network_session_.get(),
                                              std::move(backend), false)));
    }
