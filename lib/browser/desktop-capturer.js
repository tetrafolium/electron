...
const capturerResult = (id) =>
    `ELECTRON_RENDERER_DESKTOP_CAPTURER_RESULT_${id}`;
...
ipcMain.on(
    ...
    (event, captureWindow, captureScreen, thumbnailSize, id) => {
      ...
      if (requestsQueue.length === 1) {
        desktopCapturer.startHandling(
            captureWindow, captureScreen, thumbnailSize);
      }

      // If the WebContents is destroyed before receiving result, just remove
      // the reference from requestsQueue to make the module not send the result
      // to it.
...
desktopCapturer.emit = (event, name, sources) => {
  ...
  if (requestsQueue.length > 0) {
    const {
      ...
      captureScreen,
      thumbnailSize,
    } = requestsQueue[0].options;
    return desktopCapturer.startHandling(
        captureWindow, captureScreen, thumbnailSize);
  }
};
