...
const createGuest = function(embedder, params) {
  ...
  const fn = function(event) {
    guest.on(event, function(_, ...args) {
      sendToEmbedder(
          'ELECTRON_GUEST_VIEW_INTERNAL_DISPATCH_EVENT', event, ...args);
    });
  };
  ...
  guest.on('ipc-message-host', function(_, [channel, ...args]) {
    sendToEmbedder(
        'ELECTRON_GUEST_VIEW_INTERNAL_IPC_MESSAGE', channel, ...args);
  });
...
const attachGuest = function(
    event, elementInstanceId, guestInstanceId, params) {
  ...
  if (guestInstance.elementInstanceId) {
    const oldKey =
        `${guestInstance.embedder.getId()}-${guestInstance.elementInstanceId}`;
    delete embedderElementsMap[oldKey];

    // Remove guest from embedder if moving across web views
    if (guest.viewInstanceId !== params.instanceId) {
      webViewManager.removeGuest(guestInstance.embedder, guestInstanceId);
      guestInstance.embedder.send(
          `ELECTRON_GUEST_VIEW_INTERNAL_DESTROY_GUEST-${guest.viewInstanceId}`);
    }
  }

  const webPreferences = {
    guestInstanceId: guestInstanceId,
    nodeIntegration: params.nodeintegration != null ? params.nodeintegration :
                                                      false,
  ...
  webViewManager.addGuest(
      guestInstanceId, elementInstanceId, embedder, guest, webPreferences);
  ...
  const onVisibilityChange = function(visibilityState) {
    for (const guestInstanceId of Object.keys(guestInstances)) {
      ...
      if (guestInstance.embedder === embedder) {
        guestInstance.guest.send(
            'ELECTRON_GUEST_INSTANCE_VISIBILITY_CHANGE', visibilityState);
      }
    }
  };
...
ipcMain.on(
    'ELECTRON_GUEST_VIEW_MANAGER_CREATE_GUEST',
    function(event, params, requestId) {
      event.sender.send(
          `ELECTRON_RESPONSE_${requestId}`, createGuest(event.sender, params));
    });

ipcMain.on(
    'ELECTRON_GUEST_VIEW_MANAGER_ATTACH_GUEST',
    function(event, elementInstanceId, guestInstanceId, params) {
      attachGuest(event, elementInstanceId, guestInstanceId, params);
    });

ipcMain.on(
    'ELECTRON_GUEST_VIEW_MANAGER_DESTROY_GUEST',
    function(event, guestInstanceId) {
      destroyGuest(event.sender, guestInstanceId);
    });

ipcMain.on(
    'ELECTRON_GUEST_VIEW_MANAGER_SET_SIZE',
    function(event, guestInstanceId, params) {
