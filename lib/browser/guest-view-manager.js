...
const attachGuest = function (
  ...
  if (guestInstance.elementInstanceId) {
    const oldKey = `${guestInstance.embedder.getId()}-${
      guestInstance.elementInstanceId
    }`;
  ...
  const webPreferences = {
    ...
    nodeIntegration:
      params.nodeintegration != null ? params.nodeintegration : false,
...
ipcMain.on(
  ...
  function (event, params, requestId) {
    event.sender.send(
      `ELECTRON_RESPONSE_${requestId}`,
      createGuest(event.sender, params)
    );
  }
);
