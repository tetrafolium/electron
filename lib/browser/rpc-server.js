...
let getObjectMembers =
    function(object) {
  let names = Object.getOwnPropertyNames(object)
  // For Function, we should not override following properties even though they
  // are "own" properties.
  if (typeof object === 'function') {
    names = names.filter((name) => {return !FUNCTION_PROPERTIES.includes(name)})
  }
...
let getObjectPrototype =
    function(object) {
  let proto = Object.getPrototypeOf(object)
  if (proto === null || proto === Object.prototype) return null
  return {
    members: getObjectMembers(proto), proto: getObjectPrototype(proto)
  }
}

// Convert a real value into meta data.
let valueToMeta =
    function(sender, value, optimizeSimpleObject = false) {
  // Determine the type of value.
  const meta = {type: typeof value} if (meta.type === 'object') {
...
const removeRemoteListenersAndLogWarning =
    (meta, args, callIntoRenderer) => {
      ...
      if (args.length > 0 && (args[0].sender instanceof WebContents)) {
        const {sender} = args[0] const remoteEvents =
            sender.eventNames().filter(
                (eventName) => {return sender.listeners(eventName).includes(
                    callIntoRenderer)})
...
const unwrapArgs =
  ...
  const metaToValue = function(meta) {
    ...
    switch (meta.type) {
      case 'value':
        return meta.value
        case 'remote-object': return objectsRegistry.get(meta.id)
        case 'array': return unwrapArgs(sender, meta.value)
        case 'buffer': return Buffer.from(meta.value)
        case 'date': return new Date(meta.value)
        case 'promise': return Promise.resolve({then: metaToValue(meta.then)})
        case 'object': {
          let ret = {} Object.defineProperty(
              ret.constructor, 'name', {value: meta.name})
      ...
      case 'function-with-return-value':
        ...
        let callIntoRenderer = function(...args) {
          if (!sender.isDestroyed() && webContentsId === sender.getId()) {
            sender.send(
                'ELECTRON_RENDERER_CALLBACK', meta.id,
                valueToMeta(sender, args))
          } else {
...
const callFunction =
    function(event, func, caller, args) {
  ...
  try {
    ...
    throw new Error(`Could not call remote function '${
        funcName}'. Check that the function signature is correct. Underlying error: ${
        error.message}`)
  }
}
    ...
    ipcMain.on('ELECTRON_BROWSER_REQUIRE', function(event, module) {
      try {
        event.returnValue =
            valueToMeta(event.sender, process.mainModule.require(module))
      } catch (error) {
...
ipcMain.on('ELECTRON_BROWSER_CURRENT_WINDOW', function(event) {
  try {
    event.returnValue =
        valueToMeta(event.sender, event.sender.getOwnerBrowserWindow())
  } catch (error) {
    event.returnValue = exceptionToMeta(error)
  }
})
...
ipcMain.on('ELECTRON_BROWSER_CONSTRUCTOR', function(event, id, args) {
  try {
    ...
    let obj =
        new (Function.prototype.bind.apply(constructor, [null].concat(args)))()
...
ipcMain.on(
    'ELECTRON_BROWSER_MEMBER_CONSTRUCTOR', function(event, id, method, args) {
      try {
        args = unwrapArgs(event.sender, args)
        let object = objectsRegistry.get(id)

        if (object == null) {
          throwRPCError(`Cannot call constructor '${
              method}' on missing remote object ${id}`)
        }

        // Call new with array of arguments.
        let constructor = object[method] let obj = new (
            Function.prototype.bind.apply(constructor, [null].concat(args)))()
        event.returnValue = valueToMeta(event.sender, obj)
      } catch (error) {
        event.returnValue = exceptionToMeta(error)
      }
...
ipcMain.on('ELECTRON_BROWSER_MEMBER_CALL', function(event, id, method, args) {
  try {
    ...
    if (obj == null) {
      throwRPCError(
          `Cannot call function '${method}' on missing remote object ${id}`)
    }
...
ipcMain.on('ELECTRON_BROWSER_CONTEXT_RELEASE', (e) => {
  ...
  ipcMain.on(
      'ELECTRON_BROWSER_GUEST_WEB_CONTENTS', function(event, guestInstanceId) {
        try {
          let guestViewManager = require('./guest-view-manager')
          event.returnValue = valueToMeta(
              event.sender, guestViewManager.getGuest(guestInstanceId))
        } catch (error) {
          event.returnValue = exceptionToMeta(error)
        }
      })
  ...
  ipcMain.on(
      'ELECTRON_BROWSER_ASYNC_CALL_TO_GUEST_VIEW',
      function(event, requestId, guestInstanceId, method, ...args) {
        try {
          let guestViewManager = require('./guest-view-manager')
          ...
          if (requestId) {
            const responseCallback = function(result) {
              event.sender.send(
                  `ELECTRON_RENDERER_ASYNC_CALL_TO_GUEST_VIEW_RESPONSE_${
                      requestId}`,
                  result)
  ...
  ipcMain.on(
      'ELECTRON_BROWSER_SEND_TO',
      function(event, sendToAll, webContentsId, channel, ...args) {
        let contents = webContents.fromId(webContentsId)
        if (!contents) {
          console.error(
              `Sending message to WebContents with unknown ID ${webContentsId}`)
          return
        }
