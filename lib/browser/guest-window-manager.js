...
const mergeBrowserWindowOptions = function(embedder, options) {
  ...
  // Sets correct openerId here to give correct options to 'new-window' event
  // handler
...
const canAccessWindow = function(sender, target) {
  return (
      target.getWebPreferences().openerId === sender.id ||
...
ipcMain.on(
    ...
    (event, url, frameName, features) => {
          ...
          if (features == null) {
            ...
            parseFeaturesString(features, function(key, value) {
              if (value === undefined) {
                ...
                // Don't allow webPreferences to be set since it must be an
                // object that cannot be directly overridden
      ...
      ipcMain.emit(
          'ELECTRON_GUEST_WINDOW_MANAGER_INTERNAL_WINDOW_OPEN', event, url,
          frameName, disposition, options, additionalFeatures);
    });
...
ipcMain.on(
    ...
    function(
      ...
      event.sender.emit(
          'new-window', event, url, frameName, disposition, options,
          additionalFeatures);
      const {newGuest} = event;
      if ((event.sender.isGuest() && !event.sender.allowPopups) ||
          event.defaultPrevented) {
        if (newGuest != null) {
          if (options.webContents === newGuest.webContents) {
            // the webContents is not changed, so set defaultPrevented to false
            // to stop the callers of this event from destroying the
            // webContents.
            event.defaultPrevented = false;
          }
          event.returnValue =
              setupGuest(event.sender, frameName, newGuest, options);
        } else {
          event.returnValue = null;
        }
      } else {
        event.returnValue =
            createGuest(event.sender, url, frameName, options, postData);
      }
    });
    ...
    'ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_CLOSE', function(event, guestId) {
      ...
      if (guestContents == null) {
        ...
        if (!canAccessWindow(event.sender, guestContents)) {
          console.error(
              `Blocked ${event.sender.getURL()} from closing its opener.`);
          return;
        }
      }
...
ipcMain.on(
    ...
    function(event, guestId, method, ...args) {
      const guestContents = webContents.fromId(guestId);
      if (guestContents == null) {
        event.returnValue = null;
        return;
      ...
      if (!canAccessWindow(event.sender, guestContents)) {
        console.error(`Blocked ${event.sender.getURL()} from calling ${
            method} on its opener.`);
        event.returnValue = null;
        return;
      }
...
ipcMain.on(
    ...
    function(event, guestId, message, targetOrigin, sourceOrigin) {
      ...
      if (guestContents == null) {
        ...
        // The W3C does not seem to have word on how postMessage should work
        // when the origins do not match, so we do not do |canAccessWindow|
        // check here since postMessage across origins is useful and not
        // harmful.
        if (targetOrigin === '*' ||
            isSameOrigin(guestContents.getURL(), targetOrigin)) {
          const sourceId = event.sender.id;
          guestContents.send(
              'ELECTRON_GUEST_WINDOW_POSTMESSAGE', sourceId, message,
              sourceOrigin);
        }
      }
    });
