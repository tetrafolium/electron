...
ipcMain.on(
  ...
  function (
    ...
    if (
      (event.sender.isGuest() && !event.sender.allowPopups) ||
...
ipcMain.on(
  ...
  function (event, guestId, method, ...args) {
    ...
    if (!canAccessWindow(event.sender, guestContents)) {
      console.error(
        `Blocked ${event.sender.getURL()} from calling ${method} on its opener.`
      );
      event.returnValue = null;
      return;
    }
