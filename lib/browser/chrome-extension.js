...
const renderProcessPreferences = process
  .atomBinding("render_process_preferences")
  .forAllWebContents();
...
const getManifestFromPath = function (srcDirectory) {
  ...
  if (!manifestNameMap[manifest.name]) {
    ...
    console.warn(
      `Attempted to load extension "${manifest.name}" that has already been loaded.`
    );
    return manifest;
  }
};
...
ipcMain.on(
  ...
  function (event, extensionId, connectInfo) {
    ...
    page.webContents.sendToAll(
      `CHROME_RUNTIME_ONCONNECT_${extensionId}`,
      event.sender.id,
      portId,
      connectInfo
    );
...
ipcMain.on(
  ...
  function (event, extensionId, message, originResultID) {
    ...
    page.webContents.sendToAll(
      `CHROME_RUNTIME_ONMESSAGE_${extensionId}`,
      event.sender.id,
      message,
      resultID
    );
    ipcMain.once(
      `CHROME_RUNTIME_ONMESSAGE_RESULT_${resultID}`,
      (event, result) => {
        event.sender.send(
          `CHROME_RUNTIME_SENDMESSAGE_RESULT_${originResultID}`,
          result
        );
      }
    );
...
ipcMain.on(
  ...
  function (
    ...
    ipcMain.once(
      ...
      (event, result) => {
        event.sender.send(
          `CHROME_TABS_SEND_MESSAGE_RESULT_${originResultID}`,
          result
        );
      }
    );
...
ipcMain.on(
  ...
  function (event, requestId, tabId, extensionId, details) {
    ...
    if (details.file) {
      ...
      code = details.code;
      url = `chrome-extension://${extensionId}/${String(Math.random()).substr(
        2,
        8
      )}.js`;
    }
...
const loadDevToolsExtensions = function (win, manifests) {
  ...
  const chromeExtensionHandler = function (request, callback) {
    ...
    fs.readFile(
      path.join(manifest.srcDirectory, parsed.path),
...
app.on("will-quit", function () {
  try {
    const loadedDevToolsExtensions = Array.from(devToolsExtensionNames).map(
      (name) => manifestNameMap[name].srcDirectory
    );
...
app.once("ready", function () {
  ...
  loadedDevToolsExtensionsPath = path.join(
    app.getPath("userData"),
    "DevTools Extensions"
  );
  try {
    const loadedDevToolsExtensions = JSON.parse(
      fs.readFileSync(loadedDevToolsExtensionsPath)
    );
