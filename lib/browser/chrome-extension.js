...
const renderProcessPreferences =
    process.atomBinding('render_process_preferences').forAllWebContents();
...
const getManifestFromPath = function(srcDirectory) {
  ...
  if (!manifestNameMap[manifest.name]) {
    ...
    Object.assign(manifest, {
      ...
      // We can not use 'file://' directly because all resources in the
      // extension will be treated as relative to the root in Chrome.
    ...
    console.warn(`Attempted to load extension "${
        manifest.name}" that has already been loaded.`);
    return manifest;
  }
};
  ...
  if (backgroundPages[manifest.extensionId] || !manifest.background) {
    ...
    if (manifest.background.page) {
      ...
      html = fs.readFileSync(
          path.join(manifest.srcDirectory, manifest.background.page));
    } else {
...
ipcMain.on('CHROME_RUNTIME_CONNECT', function(event, extensionId, connectInfo) {
  ...
  page.webContents.sendToAll(
      `CHROME_RUNTIME_ONCONNECT_${extensionId}`, event.sender.id, portId,
      connectInfo);
});
...
ipcMain.on(
    ...
    function(event, extensionId, message, originResultID) {
      const page = backgroundPages[extensionId];
      if (!page) {
        console.error(`Connect to unknown extension ${extensionId}`);
        return;
      ...
      page.webContents.sendToAll(
          `CHROME_RUNTIME_ONMESSAGE_${extensionId}`, event.sender.id, message,
          resultID);
      ipcMain.once(
          `CHROME_RUNTIME_ONMESSAGE_RESULT_${resultID}`, (event, result) => {
            event.sender.send(
                `CHROME_RUNTIME_SENDMESSAGE_RESULT_${originResultID}`, result);
          });
      resultID++;
    });

ipcMain.on(
    'CHROME_TABS_SEND_MESSAGE',
    function(
        event, tabId, extensionId, isBackgroundPage, message, originResultID) {
      ...
      contents.sendToAll(
          `CHROME_RUNTIME_ONMESSAGE_${extensionId}`, senderTabId, message,
          resultID);
      ipcMain.once(
          `CHROME_RUNTIME_ONMESSAGE_RESULT_${resultID}`, (event, result) => {
            event.sender.send(
                `CHROME_TABS_SEND_MESSAGE_RESULT_${originResultID}`, result);
          });
      resultID++;
    });
...
ipcMain.on(
    ...
    function(event, requestId, tabId, extensionId, details) {
      const contents = webContents.fromId(tabId);
      if (!contents) {
        console.error(`Sending message to unknown tab ${tabId}`);
        return;
      ...
      if (details.file) {
        ...
        code = String(
            fs.readFileSync(path.join(manifest.srcDirectory, details.file)));
        url = `chrome-extension://${extensionId}${details.file}`;
      } else {
        code = details.code;
        url = `chrome-extension://${extensionId}/${
            String(Math.random()).substr(2, 8)}.js`;
      }

      contents.send(
          'CHROME_TABS_EXECUTESCRIPT', event.sender.id, requestId, extensionId,
          url, code);
    });
  ...
  if (contentScripts[manifest.name] || !manifest.content_scripts) {
    ...
    const readArrayOfFiles = function(relativePath) {
      return {
        ...
        code: String(
            fs.readFileSync(path.join(manifest.srcDirectory, relativePath))),
      };
    };
  }
...
const loadDevToolsExtensions = function(win, manifests) {
  if (!win.devToolsWebContents) {
    ...
    win.devToolsWebContents.executeJavaScript(
        `DevToolsAPI.addExtensions(${JSON.stringify(extensionInfoArray)})`);
  }
  ...
  const chromeExtensionHandler = function(request, callback) {
    ...
    fs.readFile(
        path.join(manifest.srcDirectory, parsed.path), function(err, content) {
          if (err) {
            // Disabled due to false positive in StandardJS
            // eslint-disable-next-line standard/no-callback-literal
...
app.on('session-created', function(ses) {
  ses.protocol.registerBufferProtocol(
      'chrome-extension', chromeExtensionHandler, function(error) {
        if (error) {
          console.error(
              `Unable to register chrome-extension protocol: ${error}`);
        }
      });
});
...
app.on('will-quit', function() {
  try {
    const loadedDevToolsExtensions =
        Array.from(devToolsExtensionNames)
    ...
    if (loadedDevToolsExtensions.length > 0) {
      ...
      fs.writeFileSync(
          loadedDevToolsExtensionsPath,
          JSON.stringify(loadedDevToolsExtensions));
    } else {
...
app.once('ready', function() {
  ...
  try {
    const loadedDevToolsExtensions =
        JSON.parse(fs.readFileSync(loadedDevToolsExtensionsPath));
