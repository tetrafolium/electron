...
class TouchBar extends EventEmitter {
    ...
    this.changeListener =
        (item) => {
          this.emit('change', item.id, item.type)
                  ...
                  this.windowListeners = {} this.items =
    {} this.ordereredItems = [] this.escapeItem = escapeItem
  ...
  _addToWindow(window) {
    ...
    if (this.windowListeners.hasOwnProperty(id)) {
      ...
      const interactionListener = (event, itemID, details) => {
        let item = this.items[itemID] if (
            item == null && this.escapeItem != null &&
    ...
    const removeListeners =
        () => {
          this.removeListener('change', changeListener) this.removeListener(
              'escape-item-change', escapeItemListener)
          window.removeListener('-touch-bar-interaction', interactionListener)
          window.removeListener('closed', removeListeners)
          window._touchBar = null delete this.windowListeners[id]
        } window.once('closed', removeListeners) this.windowListeners[id] =
            removeListeners
  ...
  _removeFromWindow(window) {
    const removeListeners =
        this.windowListeners[window.id] if (removeListeners != null) {
      removeListeners()
    }
  }
  ...
  class TouchBarItem extends EventEmitter {
    constructor() {
      super() this.id = `${nextItemID++}`
    }

    _addLiveProperty(name, initialValue) {
      const privateName = `_${name}` this[privateName] = initialValue
      Object.defineProperty(this, name, {
        get: function() {
          return this[privateName]
        },
        set: function(value) {
          this[privateName] = value this.emit('change', this)
        },
        enumerable: true
      })
    }
...
TouchBar.TouchBarButton = class TouchBarButton extends TouchBarItem {
  constructor(config) {
    super()
    if (config == null) config = {} this.type = 'button'
    const {click, icon, iconPosition, label, backgroundColor} =
        config this._addLiveProperty('label', label) this
            ._addLiveProperty('backgroundColor', backgroundColor) this
            ._addLiveProperty('icon', icon) this._addLiveProperty(
...
TouchBar.TouchBarColorPicker = class TouchBarColorPicker extends TouchBarItem {
  constructor(config) {
    super()
    if (config == null) config = {} this.type = 'colorpicker'
    const {availableColors, change, selectedColor} =
        config this._addLiveProperty('availableColors', availableColors) this
            ._addLiveProperty('selectedColor', selectedColor)
...
TouchBar.TouchBarGroup = class TouchBarGroup extends TouchBarItem {
  constructor(config) {
    super()
    if (config == null) {
      config = {} this.type = 'group' this.child = config.items
...
TouchBar.TouchBarLabel = class TouchBarLabel extends TouchBarItem {
  constructor(config) {
    ...
    if (config == null) {
      config = {} this.type =
          'label' this._addLiveProperty('label', config.label) this
...
TouchBar.TouchBarPopover = class TouchBarPopover extends TouchBarItem {
  constructor(config) {
    ...
    if (config == null) {
      config = {} this.type =
          'popover' this._addLiveProperty('label', config.label) this
              ._addLiveProperty('icon', config.icon) this.showCloseButton =
              config.showCloseButton this.child = config.items
      if (!(this.child instanceof TouchBar)) {
        this.child = new TouchBar(this.child)
      }
    }
    this.child.ordereredItems.forEach((item) => {
      item._popover =
          item._popover || [] if (!item._popover.includes(this.id)) {
        item._popover.push(this.id)
      })
  }
  }
}
...
TouchBar.TouchBarSlider = class TouchBarSlider extends TouchBarItem {
  constructor(config) {
    super()
    if (config == null) config = {} this.type = 'slider'
    const {change, label, minValue, maxValue, value} =
        config this._addLiveProperty(
                       'label',
                       label) this._addLiveProperty('minValue', minValue) this
            ._addLiveProperty('maxValue', maxValue) this._addLiveProperty(
                'value', value)
...
TouchBar.TouchBarSpacer = class TouchBarSpacer extends TouchBarItem {
  constructor(config) {
    super()
    if (config == null) {
      config = {} this.type = 'spacer' this.size = config.size
    }
  }
}
...
TouchBar.TouchBarSegmentedControl =
  ...
  constructor(config) {
    ...
    {} const {segmentStyle, segments, selectedIndex, change, mode} =
        config this.type =
            'segmented_control' this
                ._addLiveProperty('segmentStyle', segmentStyle) this
                ._addLiveProperty('segments', segments || []) this
                ._addLiveProperty(
                    'selectedIndex',
                    selectedIndex) this._addLiveProperty('mode', mode)
...
TouchBar.TouchBarScrubber = class TouchBarScrubber extends TouchBarItem {
  constructor(config) {
    ...
    let {select, highlight} = config this.type =
        'scrubber' this._addLiveProperty('items', items) this
            ._addLiveProperty('selectedStyle', selectedStyle || null) this
            ._addLiveProperty('overlayStyle', overlayStyle || null) this
            ._addLiveProperty(
                'showArrowButtons', showArrowButtons || false) this
            ._addLiveProperty('mode', mode || 'free') this._addLiveProperty(
                'continuous',
                typeof continuous === 'undefined' ? true : continuous)
