...
const asyncWebFrameMethods = function(requestId, method, callback, ...args) {
  return new Promise((resolve, reject) => {
    this.send(
        'ELECTRON_INTERNAL_RENDERER_ASYNC_WEB_FRAME_METHOD', requestId, method,
        args);
    ipcMain.once(
        `ELECTRON_INTERNAL_BROWSER_ASYNC_WEB_FRAME_RESPONSE_${requestId}`,
        function(event, error, result) {
          if (error == null) {
            ...
            if (error.__ELECTRON_SERIALIZED_ERROR__ &&
                errorConstructors[error.name]) {
              const rehydratedError =
                  new errorConstructors[error.name](error.message);
...
const syncWebFrameMethods = function(requestId, method, callback, ...args) {
  this.send(
      'ELECTRON_INTERNAL_RENDERER_SYNC_WEB_FRAME_METHOD', requestId, method,
      args);
  ipcMain.once(
      `ELECTRON_INTERNAL_BROWSER_SYNC_WEB_FRAME_RESPONSE_${requestId}`,
...
for (const method of webFrameMethodsWithResult) {
  WebContents.prototype[method] = function(...args) {
    ...
    syncWebFrameMethods.call(
        this, getNextId(), method, callback, ...actualArgs);
  };
}

// Make sure WebContents::executeJavaScript would run the code only when the
// WebContents has been loaded.
WebContents.prototype.executeJavaScript = function(
    code, hasUserGesture, callback) {
  ...
  if (this.getURL() && !this.isLoadingMainFrame()) {
    return asyncWebFrameMethods.call(
        this, requestId, 'executeJavaScript', callback, code, hasUserGesture);
  } else {
    return new Promise((resolve, reject) => {
      this.once('did-finish-load', () => {
        asyncWebFrameMethods
            .call(
                this, requestId, 'executeJavaScript', callback, code,
                hasUserGesture)
            .then(resolve)
            .catch(reject);
      });
