...
Menu.prototype._init =
    function() {
  this.commandsMap = {} this.groupsMap = {} this.items = [] this.delegate = {
    isCommandIdChecked: id =>
        this.commandsMap[id] ? this.commandsMap[id].checked : undefined,
    isCommandIdEnabled: id =>
        this.commandsMap[id] ? this.commandsMap[id].enabled : undefined,
    isCommandIdVisible: id =>
        this.commandsMap[id] ? this.commandsMap[id].visible : undefined,
    getAcceleratorForCommandId: (id, useDefaultAccelerator) => {
      const command = this.commandsMap[id] if (!command) {
        return if (command.accelerator) {
      ...
      ,
          getIconForCommandId:
              id =>
                  this.commandsMap[id] ? this.commandsMap[id].icon : undefined,
              executeCommand: (event, id) => {
                const command = this.commandsMap[id] if (!command) {
                  return command.click(
                      event, BrowserWindow.getFocusedWindow(),
                ...
                for (const id in this.groupsMap) {
                  const found =
                      this.groupsMap[id].find(item => item.checked) || null
                  if (!found) {
                    v8Util.setHiddenValue(
                        this.groupsMap[id][0], 'checked', true)
                  }
                }
              }
    },
    ...
    Menu.prototype.popup =
        function(window, x, y, positioningItem) {
  let [newX, newY, newPosition, newWindow] =
      [x, y, positioningItem, window] let opts
  ...
  if (typeof newX !== 'number') {
    ...
    if (typeof newY !== 'number') {
      ...
      if (typeof newPosition !== 'number') {
        newPosition = -1
        if (!newWindow ||
            (newWindow && newWindow.constructor !== BrowserWindow)) {
          ...
          if (!newWindow) {
            ...
            if (browserWindows && browserWindows.length > 0) {
              ...
              throw new Error(
                  `Cannot open Menu without a BrowserWindow present`)
            }
          }
        }
      }
  ...
  return {
    browserWindow: newWindow, x: newX, y: newY, position: newPosition
  }
}

        Menu.prototype.closePopup =
            function(window) {
  if (window && window.constructor !== BrowserWindow) {
    ...
    this.closePopupAt(-1)
            ...
            Menu.prototype.getMenuItemById =
                function(id) {
    ...
    if (item.icon) {
      ...
      if (item.role) {
        ...
        item.overrideReadOnlyProperty('menu', this)
            ...
            // Remember the items.
            this.items.splice(pos, 0, item) this.commandsMap[item.commandId] =
            item
      }
    }
  }
                        ...
                        Menu.prototype._callMenuWillShow =
                            function() {
...
Menu.setApplicationMenu =
  ...
  template.forEach(item => {
    ...
    return menu
        ...
        // Search between separators to find a radio menu item and return its
        // group id
        ...
        function indexToInsertByPosition(items, position) {
          if (!position) {
            ...
            const [query, id] =
                position.split('=')  // parse query and id from position
            const idx =
                indexOfItemById(items, id)  // calculate initial index of item
        ...
        function insertItemByType(item, pos) {
          const types = {
            ...
            submenu: () => this.insertSubMenu(
                pos, item.commandId, item.label, item.submenu),
            radio: () => {
              // Grouping radio menu items
              item.overrideReadOnlyProperty(
                  'groupId', generateGroupId(this.items, pos))
