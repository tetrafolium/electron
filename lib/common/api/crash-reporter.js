...
const {app, deprecate} =
    process.type === 'browser' ? electron : electron.remote;
...
class CrashReporter {
  start(options) {
    if (options == null)
      options = {};
    this.productName =
        options.productName != null ? options.productName : app.getName();
    ...
    binding.start(
        this.getProductName(), companyName, submitURL,
        this.getCrashesDirectory(), uploadToServer, ignoreSystemCrashHandler,
        extra);
  }
  ...
  getUploadToServer() {
    if (process.type === 'browser') {
      ...
      throw new Error(
          'getUploadToServer can only be called from the main process');
    }
  }
  ...
  setUploadToServer(uploadToServer) {
    if (process.type === 'browser') {
      ...
      throw new Error(
          'setUploadToServer can only be called from the main process');
    }
  }
