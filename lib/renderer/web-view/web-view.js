...
class WebViewImpl {
  constructor(webviewNode) {
    ...
    shadowRoot.innerHTML =
        '<!DOCTYPE html><style type="text/css">:host { display: flex; }</style>';
  ...
  reset() {
    ...
    // already picked up a partition ID. Thus, we need to reset the
    // initialization state. However, it may be the case that
    // beforeFirstNavigation is false BUT guestInstanceId has yet to be
    // initialized. This means that we have not heard back from createGuest yet.
    // We will not reset the flag in this case so that we don't end up
    // allocating a second guest.
    ...
    // Set guestinstance last since this can trigger the attachedCallback to
    // fire when moving the webview using element.replaceChild
    this.attributes[webViewConstants.ATTRIBUTE_GUESTINSTANCE]
        .setValueIgnoreMutation(undefined);
  }
    ...
    if (attributeName === webViewConstants.ATTRIBUTE_INTERNALINSTANCEID &&
      ...
      if (this.guestInstanceId) {
        guestViewInternal.attachGuest(
            this.internalInstanceId, this.guestInstanceId, this.buildParams());
      }
    }
  }
  ...
  onSizeChanged(webViewEvent) {
    ...
    const maxWidth =
        this.attributes[webViewConstants.ATTRIBUTE_MAXWIDTH].getValue() | width;
    const maxHeight =
        this.attributes[webViewConstants.ATTRIBUTE_MAXHEIGHT].getValue() |
        width;
    let minWidth =
        this.attributes[webViewConstants.ATTRIBUTE_MINWIDTH].getValue() | width;
    let minHeight =
        this.attributes[webViewConstants.ATTRIBUTE_MINHEIGHT].getValue() |
        width;
  ...
  createGuest() {
    return guestViewInternal.createGuest(
        this.buildParams(), (event, guestInstanceId) => {
    ...
    return Object.defineProperty(this.webviewNode, propertyName, {
      ...
      set: (value) => {
        if (this.on[propertyName]) {
          this.webviewNode.removeEventListener(
  ...
  onAttach(storagePartitionId) {
    return this.attributes[webViewConstants.ATTRIBUTE_PARTITION].setValue(
        storagePartitionId);
  }
  ...
  attachGuestInstance(guestInstanceId) {
    ...
    this.attributes[webViewConstants.ATTRIBUTE_GUESTINSTANCE]
        .setValueIgnoreMutation(guestInstanceId);
    this.webContents = remote.getGuestWebContents(this.guestInstanceId);
    if (!this.internalInstanceId) {
      return true;
    }
    return guestViewInternal.attachGuest(
        this.internalInstanceId, this.guestInstanceId, this.buildParams());
  }
}
...
const registerBrowserPluginElement = function() {
  ...
  WebViewImpl.BrowserPlugin =
      webFrame.registerEmbedderCustomElement('browserplugin', {
        extends: 'object',
        prototype: proto,
      });
...
const registerWebViewElement = function() {
  ...
  proto.attachedCallback = function() {
    ...
    if (!internal.elementAttached) {
      ...
      const instance =
          internal.attributes[webViewConstants.ATTRIBUTE_GUESTINSTANCE]
              .getValue();
      if (instance) {
    ...
    return function(...args) {
      ...
      if (internal.webContents) {
        ...
        throw new Error(`Cannot call ${
            m} because the webContents is unavailable. The WebView must be attached to the DOM and the dom-ready event emitted before this method can be called.`);
      }
    };
  };
  ...
  const createNonBlockHandler = function(m) {
    return function(...args) {
      const internal = v8Util.getHiddenValue(this, 'internal');
      ipcRenderer.send(
          'ELECTRON_BROWSER_ASYNC_CALL_TO_GUEST_VIEW', null,
          internal.guestInstanceId, m, ...args);
    };
  };
  ...
  proto.executeJavaScript = function(code, hasUserGesture, callback) {
    ...
    ipcRenderer.send(
        'ELECTRON_BROWSER_ASYNC_CALL_TO_GUEST_VIEW', requestId,
        internal.guestInstanceId, 'executeJavaScript', code, hasUserGesture);
    ipcRenderer.once(
        `ELECTRON_RENDERER_ASYNC_CALL_TO_GUEST_VIEW_RESPONSE_${requestId}`,
