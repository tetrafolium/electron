...
class WebViewImpl {
  ...
  reset() {
    ...
    this.attributes[
      webViewConstants.ATTRIBUTE_GUESTINSTANCE
  ...
  handleBrowserPluginAttributeMutation(attributeName, oldValue, newValue) {
    if (
      attributeName === webViewConstants.ATTRIBUTE_INTERNALINSTANCEID &&
      !oldValue &&
      !!newValue
    ) {
  ...
  onSizeChanged(webViewEvent) {
    ...
    const maxHeight =
      this.attributes[webViewConstants.ATTRIBUTE_MAXHEIGHT].getValue() | width;
    let minWidth =
      this.attributes[webViewConstants.ATTRIBUTE_MINWIDTH].getValue() | width;
    let minHeight =
      this.attributes[webViewConstants.ATTRIBUTE_MINHEIGHT].getValue() | width;
    minWidth = Math.min(minWidth, maxWidth);
    minHeight = Math.min(minHeight, maxHeight);
    if (
      !this.attributes[webViewConstants.ATTRIBUTE_AUTOSIZE].getValue() ||
      (newWidth >= minWidth &&
        newWidth <= maxWidth &&
        newHeight >= minHeight &&
        newHeight <= maxHeight)
  ...
  onLoadCommit(webViewEvent) {
    const oldValue = this.webviewNode.getAttribute(
  ...
  buildParams() {
    ...
    params.elementHeight =
      parseInt(elementRect.height) || parseInt(css.getPropertyValue("height"));
    return params;
  }
...
const registerBrowserPluginElement = function () {
  ...
  WebViewImpl.BrowserPlugin = webFrame.registerEmbedderCustomElement(
    "browserplugin",
    {
      extends: "object",
      prototype: proto,
...
const registerWebViewElement = function () {
  ...
  proto.attachedCallback = function () {
    ...
    if (!internal.elementAttached) {
      ...
      const instance = internal.attributes[
        webViewConstants.ATTRIBUTE_GUESTINSTANCE
      ].getValue();
      if (instance) {
  ...
  const createBlockHandler = function (m) {
    return function (...args) {
      ...
      if (internal.webContents) {
        ...
        throw new Error(
          `Cannot call ${m} because the webContents is unavailable. The WebView must be attached to the DOM and the dom-ready event emitted before this method can be called.`
        );
      }
    };
  };
