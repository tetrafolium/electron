...
module.exports = {
  registerEvents: function(webView, viewInstanceId) {
    ipcRenderer.on(
        `ELECTRON_GUEST_VIEW_INTERNAL_DESTROY_GUEST-${viewInstanceId}`,
        function() {
    ...
    ipcRenderer.on(
        `ELECTRON_GUEST_VIEW_INTERNAL_DISPATCH_EVENT-${viewInstanceId}`,
        function(event, eventName, ...args) {
          dispatchEvent(webView, eventName, eventName, ...args);
        });

    ipcRenderer.on(
        `ELECTRON_GUEST_VIEW_INTERNAL_IPC_MESSAGE-${viewInstanceId}`,
    ...
    ipcRenderer.on(
        `ELECTRON_GUEST_VIEW_INTERNAL_SIZE_CHANGED-${viewInstanceId}`,
        function(event, ...args) {
  ...
  createGuest: function(params, callback) {
    requestId++;
    ipcRenderer.send(
        'ELECTRON_GUEST_VIEW_MANAGER_CREATE_GUEST', params, requestId);
    ipcRenderer.once(`ELECTRON_RESPONSE_${requestId}`, callback);
  },
  attachGuest: function(elementInstanceId, guestInstanceId, params) {
    ipcRenderer.send(
        'ELECTRON_GUEST_VIEW_MANAGER_ATTACH_GUEST', elementInstanceId,
        guestInstanceId, params);
    webFrame.attachGuest(elementInstanceId);
  },
  destroyGuest: function(guestInstanceId) {
    ipcRenderer.send(
        'ELECTRON_GUEST_VIEW_MANAGER_DESTROY_GUEST', guestInstanceId);
  },
  setSize: function(guestInstanceId, params) {
    ipcRenderer.send(
        'ELECTRON_GUEST_VIEW_MANAGER_SET_SIZE', guestInstanceId, params);
  },
};
