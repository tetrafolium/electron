...
class SrcAttribute extends WebViewAttribute {
  ...
  parse() {
    ...
    const opts = {};
    const httpreferrer = this.webViewImpl.attributes[
      webViewConstants.ATTRIBUTE_HTTPREFERRER
    ].getValue();
    if (httpreferrer) {
      opts.httpReferrer = httpreferrer;
    }
    const useragent = this.webViewImpl.attributes[
      webViewConstants.ATTRIBUTE_USERAGENT
    ].getValue();
    if (useragent) {
      opts.userAgent = useragent;
    }
    const guestContents = remote.getGuestWebContents(
...
class PreloadAttribute extends WebViewAttribute {
  ...
  getValue() {
    ...
    let preload = resolveURL(
      this.webViewImpl.webviewNode.getAttribute(this.name)
    );
...
WebViewImpl.prototype.setupWebViewAttributes = function () {
  this.attributes = {};
  this.attributes[webViewConstants.ATTRIBUTE_AUTOSIZE] = new AutosizeAttribute(
    this
  );
  this.attributes[
  ...
  this.attributes[webViewConstants.ATTRIBUTE_PLUGINS] = new BooleanAttribute(
  ...
  this.attributes[webViewConstants.ATTRIBUTE_PRELOAD] = new PreloadAttribute(
    this
  );
  this.attributes[
  ...
  this.attributes[
    webViewConstants.ATTRIBUTE_DISABLEGUESTRESIZE
  ...
  autosizeAttributes.forEach((attribute) => {
    this.attributes[attribute] = new AutosizeDimensionAttribute(
      attribute,
      this
    );
  });
