...
function BrowserWindowProxy(ipcRenderer, guestId) {
  ...
  defineProperty(this, 'location', {
    get: function() {
      return ipcRenderer.sendSync(
          'ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD_SYNC', guestId,
          'getURL');
    },
    set: function(url) {
      url = resolveURL(url);
  ...
  this.focus = () => {
    ipcRenderer.send(
        'ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_METHOD', guestId, 'focus');
  };

  this.blur = () => {
    ipcRenderer.send(
        'ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_METHOD', guestId, 'blur');
  };

  this.print = () => {
    ipcRenderer.send(
        'ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', guestId, 'print');
  };

  this.postMessage = (message, targetOrigin) => {
    ipcRenderer.send(
        'ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_POSTMESSAGE', guestId, message,
        toString(targetOrigin), window.location.origin);
  };

  this.eval = (...args) => {
    ipcRenderer.send(
        'ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', guestId,
     ...
     usesNativeWindowOpen) => {
      ...
      if (!usesNativeWindowOpen) {
        ...
        window.open = function(url, frameName, features) {
          ...
          const guestId = ipcRenderer.sendSync(
              'ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_OPEN', url,
              toString(frameName), toString(features));
          if (guestId != null) {
      ...
      window.alert = function(message, title) {
        ipcRenderer.sendSync(
            'ELECTRON_BROWSER_WINDOW_ALERT', toString(message),
            toString(title));
      };

      window.confirm = function(message, title) {
        return ipcRenderer.sendSync(
            'ELECTRON_BROWSER_WINDOW_CONFIRM', toString(message),
            toString(title));
      };
      ...
      ipcRenderer.on(
          ...
          function(event, sourceId, message, sourceOrigin) {
            // Manually dispatch event instead of using postMessage because we
            // also need to set event.source.
      ...
      if (guestInstanceId != null) {
        // Webview `document.visibilityState` tracks window visibility (and
        // ignores the actual <webview> element visibility) for backwards
        // compatibility. See discussion in #9178.
        //
        // Note that this results in duplicate visibilitychange events (since
        // Chromium also fires them) and potentially incorrect visibility
        // change. We should reconsider this decision for Electron 2.0.
        ...
        // Make document.hidden and document.visibilityState return the correct
        // value.
