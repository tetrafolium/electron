...
const getChromeStoragePath =
    (storageType, extensionId) => {
      return path.join(
          app.getPath('userData'),
          `/Chrome Storage/${extensionId}-${storageType}.json`)
    }
...
const readChromeStorageFile =
    (storageType, extensionId, cb) => {
...
const writeChromeStorageFile =
    (storageType, extensionId, data, cb) => {
      const filePath = getChromeStoragePath(storageType, extensionId)
    ...
    return {
      ...
      getStorage(storageType, extensionId, storage => {
          if (keys == null) {
            ...
            let defaults = {} switch (typeof keys) {
              case 'string':
                keys = [keys] break case 'object': if (!Array.isArray(keys)) {
                  defaults = keys
          ...
          if (keys.length === 0) {
            ...
            let items = {} keys.forEach(function(key) {
              var value = storage[key] if (value == null) {
                value = defaults[key]
                items[key] = value
              }
            })
