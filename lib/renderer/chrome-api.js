...
class Port {
  ...
  disconnect() {
    if (this.disconnected) {
      ...
      ipcRenderer.sendToAll(
          this.tabId, `CHROME_PORT_DISCONNECT_${this.portId}`);
      this._onDisconnect();
    }
  }
  ...
  postMessage(message) {
    ipcRenderer.sendToAll(
        this.tabId, `CHROME_PORT_POSTMESSAGE_${this.portId}`, message);
  }
...
exports.injectTo = function(extensionId, isBackgroundPage, context) {
  ...
  ipcRenderer.on(
      ...
      (event, tabId, portId, connectInfo) => {
        chrome.runtime.onConnect.emit(
            new Port(tabId, portId, extensionId, connectInfo.name));
      });

  ipcRenderer.on(
      `CHROME_RUNTIME_ONMESSAGE_${extensionId}`,
      (event, tabId, message, resultID) => {
        chrome.runtime.onMessage.emit(
            message, new MessageSender(tabId, extensionId), (messageResult) => {
              ipcRenderer.send(
                  `CHROME_RUNTIME_ONMESSAGE_RESULT_${resultID}`, messageResult);
            });
      });
  ...
  chrome.runtime = {
    ...
    connect(...args) {
      if (isBackgroundPage) {
        console.error(
            'chrome.runtime.connect is not supported in background page');
        return;
      }
      ...
      const {tabId, portId} = ipcRenderer.sendSync(
          'CHROME_RUNTIME_CONNECT', targetExtensionId, connectInfo);
    ...
    sendMessage(...args) {
      if (isBackgroundPage) {
        console.error(
            'chrome.runtime.sendMessage is not supported in background page');
        return;
      }
      ...
      if (args.length === 1) {
        ...
        ipcRenderer.on(
            `CHROME_RUNTIME_SENDMESSAGE_RESULT_${originResultID}`,
            (event, result) => args[2](result));
      }

      ipcRenderer.send(
          'CHROME_RUNTIME_SENDMESSAGE', targetExtensionId, message,
          originResultID);
      originResultID++;
    },
  ...
  chrome.tabs = {
    executeScript(tabId, details, callback) {
      ...
      ipcRenderer.once(
          `CHROME_TABS_EXECUTESCRIPT_RESULT_${requestId}`, (event, result) => {
            // Disabled due to false positive in StandardJS
            // eslint-disable-next-line standard/no-callback-literal
            callback([event.result]);
          });
      ipcRenderer.send(
          'CHROME_TABS_EXECUTESCRIPT', requestId, tabId, extensionId, details);
    },
    ...
    sendMessage(tabId, message, options, responseCallback) {
      if (responseCallback) {
        ipcRenderer.on(
            ...
            (event, result) => responseCallback(result));
      }
      ipcRenderer.send(
          'CHROME_TABS_SEND_MESSAGE', tabId, extensionId, isBackgroundPage,
          message, originResultID);
      originResultID++;
    },
