...
ipcRenderer.sendTo = function(webContentsId, channel, ...args) {
  ...
  ipcRenderer.send(
      'ELECTRON_BROWSER_SEND_TO', false, webContentsId, channel, ...args);
};
...
ipcRenderer.sendToAll = function(webContentsId, channel, ...args) {
  ...
  ipcRenderer.send(
      'ELECTRON_BROWSER_SEND_TO', true, webContentsId, channel, ...args);
};
...
ipcRenderer.removeAllListeners = function(...args) {
  if (args.length === 0) {
    throw new Error(
        'Removing all listeners from ipcRenderer will make Electron internals stop working.  Please specify a event name');
  }
