...
function wrapArgs(args, visited = new Set()) {
  const valueToMeta = (value) => {
    ...
    if (Array.isArray(value)) {
      ...
      return {
        type: 'buffer', value: Buffer.from(value)
      }
      ...
      if (isPromise(value)) {
        return {
          type: 'promise', then: valueToMeta(function(onFulfilled, onRejected) {
      ...
      for (let prop in value) {
        meta.members.push({name: prop, value: valueToMeta(value[prop])})
      }
      visited.delete(value)
      return meta
    } else if (
        typeof value === 'function' &&
        v8Util.getHiddenValue(value, 'returnValue')) {
      return {
      ...
      if (object.hasOwnProperty(member.name))
        continue
          ...
          const remoteMemberFunction =
            ...
            const ret = ipcRenderer.sendSync(
                command, metaId, member.name, wrapArgs(args))
            return metaToValue(ret)
          }

          let descriptorFunction =
              proxyFunctionProperties(remoteMemberFunction, metaId, member.name)

          descriptor.get =
              () => {
                descriptorFunction.ref =
                    ref  // The member should reference its object.
        ...
        if (member.writable) {
          descriptor.set = (value) => {
            ...
            const meta =
                ipcRenderer.sendSync(command, metaId, member.name, args)
            if (meta != null) {
              metaToValue(meta)
              return value
            }
...
function setObjectPrototype(ref, object, metaId, descriptor) {
  if (descriptor === null) {
    return let proto = {} setObjectMembers(
               ref, proto, metaId, descriptor.members)
...
function proxyFunctionProperties(remoteMemberFunction, metaId, name) {
  ...
  const loadRemoteProperties = () => {
    if (loaded) {
      ...
      setObjectMembers(
          remoteMemberFunction, remoteMemberFunction, meta.id, meta.members)
    }
    ...
    set: (target, property, value, receiver) => {
        ...
        , get: (target, property, receiver) => {
          if (!target.hasOwnProperty(property)) {
            loadRemoteProperties()
            const value = target[property] if (
                property === 'toString' && typeof value === 'function') {
...
function metaToValue(meta) {
  ...
  if (meta.type in types) {
    ...
    // Track delegate obj's lifetime & tell browser to clean up when object is
    // GCed.
...
ipcRenderer.on(
    ...
    (event, id, args) => {callbacksRegistry.apply(id, metaToValue(args))})

// A callback in browser is released.
ipcRenderer.on(
    'ELECTRON_RENDERER_RELEASE_CALLBACK',
    (event, id) => {callbacksRegistry.remove(id)})
...
process.on('exit', () => {
                ...
                return metaToValue(meta)
                  ...
                  () => {
                    return metaToValue(ipcRenderer.sendSync(
                        'ELECTRON_BROWSER_CURRENT_WEB_CONTENTS'))
                  }
                      ...
                      (name) => {
                        ...
                        return metaToValue(meta)
                                ...
                                exports.__defineGetter__(
                                    'process',
                                    () => exports.getGlobal('process'))

  // Create a function that will return the specified value when called in
  // browser.
  exports.createFunctionWithReturnValue =
      (returnValue) => {
  ...
  const addBuiltinProperty =
      (name) => {
        Object.defineProperty(
            exports, name, {get: () => exports.getBuiltin(name)})
      }
  ...
  browserModules.filter((m) => !m.private)
      .map((m) => m.name)
